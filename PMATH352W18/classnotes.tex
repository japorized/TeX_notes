...
\begin{note}
 ...
 \begin{center}
 	\begin{tabular}{r@{\;{=}\;}l r@{\;{=}\;}l r@{\;{=}\;}l}
 		...
 		$\bar{zw}$		& $\bar{z}\bar{w}$	& $\abs{zw}$			&	$\abs{z}\abs{w}$	& $\bar{z}^n$ & $\bar{z^n}$
 	\end{tabular}
 \end{center}
 but note that $\abs{z + w} \neq \abs{z} + \abs{w}$.

 Also, note that the last equation is a generalization of the last third equation.
\end{note}
...
It is natural to wonder if the converse of \cref{thm:cauchy_riemann_equations} is true. We present the following example.
...
\begin{defn}[Power Series]\label{defn:power_series}
	A \textbf{power series} in $\mathbb{C}$ is an infinite series of the form
...
These notions, in turn, begs the question of what is the largest possible $\abs{z_0}$ for the series to converge absolutely.
