...
\begin{proof}
	...
	\end{equation*}
	and $\ell(\Delta_i^{(j)}) = \frac{1}{2^j} \ell (\Delta)$. From real analysis, $\exists z_0 \in \mathbb{C}$ such that $z_0 \in \Delta_i^{(j)}$ for all $j \in \mathbb{N} \setminus \{0\}$ that is a limit point. Since $z_0 \in \Omega \; \land \; f \in H(\Omega)$, we have that
	\begin{gather*}
	...
	\hlwarn{(I am still trying to understand the reason behind the steps from hereon)}

	By \cref{item:absval_integral_less_than_integral_absval},
	\begin{align*}
		\abs{\int_{\Delta_1^{(n)}} f(z) dz}
			&\leq \int_{\Delta_1^{(n)}} \abs{f(z) - f(z_0) - f'(z_0)(z - z_0)} dz \\
			&\leq \int_{\Delta_1^{(n)}} \epsilon \abs{z - z_0} dz \leq \epsilon \ell(\Delta_1^{(n)})\int_{\Delta_1^{(n)}} dz
	\end{align*}
	which implies that
	\begin{align*}
		\frac{\abs{J}}{4^n} \leq \epsilon \ell(\Delta_1^{(n)})^2 \leq \epsilon \frac{\ell(\Delta)^2}{4} = \abs{J} \leq \epsilon
	\end{align*}
...
\begin{enumerate}
	...
	\item Let $\{c_n\}_{n = 0}^\infty$ be a sequence of positive real numbers such that
		...
		\begin{solution}
			\hlwarn{no solution yet}

			Current ideas: If the sequence is a constant sequence, then L is 1 and thus the proof is trivial. Will have to look at the other two cases.
		\end{solution}
