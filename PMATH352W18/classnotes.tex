...
\begin{eg}
	Let $f(z) = f(x + iy) = x^2 + y^2$ be continuous along $\gamma : [0, 1] \to \mathbb{C} \enspace t \mapsto t + it$. Evaluate $\int_{\gamma} f(z) dz$.
	\begin{solution}
		\begin{align*}
			\int_{\gamma} f(z) dz
...
\begin{eg}\label{eg:integral_of_path_on_a_circle}
	$\forall n \in \mathbb{Z}$, evaluate $\int_{\gamma} z^n dz$ that is continue on the path $\gamma$ that describes any circle centered at origin oriented anticlockwise.
...
\begin{propo}[Properties of integrals in $\mathbb{C}$]\label{propo:properties_of_integrals_in_c}
	\begin{enumerate}
		\item \hlnotea{(Linearity)} Let $\alpha, \beta \in \mathbb{C}$. $\int_{\gamma} \left(\alpha f(z) + \beta g(z) \right) dz = \alpha \int_{\gamma} f(z) dz + \beta \int_{\gamma} g(z) dz$. \label{item:linearity_property_of_integrals}
		\item \begin{enumerate}
			...
			\item For any function $f(z)$ that is continuous on a path $\gamma: [a, b] \to \mathbb{C}$,
					\begin{equation*}
						\abs{\int_{\gamma} f(z) dz} \leq \sup_{z \in \gamma} \abs{f(z)} \cdot \underbrace{\int_{a}^{b} \abs{\gamma'(t)} dt}_{\text{length of the path}}
...
\begin{proof}
	\begin{enumerate}
		\item \begin{align*}
			\text{LHS}=&\int_{\gamma} \alpha f(z) + \beta g(z) dz \\
				= &\int_{\gamma} \alpha \Big( \re(f(z)) + i \im(f(z)) \Big) + \beta \Big( \re(g(z)) + i \im(g(z)) \Big) dz \\
				= \begin{split}
				...
				= & \alpha \int_{a}^{b} f(z) dz + \beta \int_{a}^{b} g(z) dz
			\end{align*}
...
\begin{proof}
	A closed path $\gamma: [a, b] \to \mathbb{C}$ has $\gamma(a) = \gamma(b)$. By \cref{thm:fundamental_theorem_of_calculus}, $\int_{\gamma} F'(z) dz = F(\gamma(b)) - F(\gamma(a)) = 0$ as required. \qed
\end{proof}

\begin{eg}
	Take $f(z) = z^n$ where $n \in \mathbb{Z} \setminus \{-1\}$ as in \cref{eg:integral_of_path_on_a_circle}. Then $f$ is continuous on $\mathbb{C} \setminus \{0\}$ \hlwarn{(not sure why this would be problematic when we've already excluded -1 for n)}. Then $f = F'$ for $F(z) = \frac{z^{n + 1}}{n + 1}$ and $F \in H(\mathbb{C} \setminus \{0\})$. Therefore by \cref{crly:corollary_of_ftc}, $\int_{\gamma} z^n dz = 0$ for any closed path $\gamma$ not passing through $0$.

	If we do include $-1$ for $n$, note that $F'$ would not be continuous on $0$, and thus the corollary would not apply. We have also shown in the earlier example that $\int_{\gamma} \frac{1}{z} dz = 2 \pi i$.
\end{eg}

\begin{note}[Recall]
	The \hldefn{interior} of a set $\Omega$ is defined as $\{z \in \Omega : \forall \epsilon > 0 \; B(z, \epsilon) \subseteq \Omega\}$.
\end{note}
...
\begin{proof}
	...
	\begin{equation}\label{eq:goursat_eq1}
		J := \int_{\Delta} f(z) dz = \sum_{i=1}^{4} \int_{\Delta_i^{(1)}} f(z) dz 
	\end{equation}
	...
	\begin{equation*}
		\abs{\int_{\Delta_i^{(j)}} f(z) dz} \geq \frac{\abs{J}}{4^j} 
	\end{equation*}
	...
	\begin{equation*}
		\int_{\Delta_1^{(n)}} f(z) dz = \int_{\Delta_1^{(n)}} \big(f(z) - f(z_0) - f'(z_0)(z - z_0)\big) dz,
	\end{equation*}
	where we note that $\int_{\Delta_1^{(n)}} f(z_0) dz = \int_{\Delta_1^{(n)}} f'(z_0)(z - z_0) dz = 0$ by \cref{crly:corollary_of_ftc}.

	By \cref{item:absval_integral_less_than_integral_absval} in \cref{propo:properties_of_integrals_in_c},
	\begin{align*}
		\abs{\int_{\Delta_1^{(n)}} f(z) dz}
			&\leq \int_{\Delta_1^{(n)}} \abs{f(z) - f(z_0) - f'(z_0)(z - z_0)} dz \\
			&\leq \int_{\Delta_1^{(n)}} \epsilon \abs{z - z_0} dz \leq \epsilon \ell(\Delta_1^{(n)})\int_{\Delta_1^{(n)}} dz = \epsilon \ell(\Delta_1^{(n)})^2
	\end{align*}
...
\addcontentsline{toc}{chapter}{Tutorial}
	\label{tutorial:tutorial_jan_31_2018}
...
\begin{enumerate}
	...
	\item If the power series $f(z) = \sum_{n=0}^{\infty} c_n (z - z_0)^n$ centered at $z_0$ has a non-zero radius of convergence, then show that
		\begin{equation*}
			c_m = \frac{f^{(m)}(z_0)}{m!}
		\end{equation*}
		for any $m \in \mathbb{Z}, m \geq 0,$ where $f^{(m)}(z_0)$ denotes the $m$th derivative of $f$ at $z_0$.
	...
	\item Evaluate the above integral by finding an antiderivative. (Hint: Use $\left(\frac{z\bar{z}}{z} \right)^2$)
		\begin{solution}
			...
			\begin{equation*}
				\int_{\gamma} \bar{z}^2 dz = \int_{\gamma} \frac{1}{z^2} = F(\gamma(0)) - F\left(\gamma\left(-\frac{\pi}{2}\right)\right) = -\frac{1}{e^{-i(0)}} + \frac{1}{e^{-i(-\pi/2)}} = - 1 - i 
			\end{equation*}
		\end{solution}
	...
	\item Show that for any path $\gamma: [a, b] \to \mathbb{C}$ and $f(z)$ continuous on $\gamma$, we have
		\begin{equation*}
			\abs{\int_{\gamma} f(z) dz} \leq \sup_{z \in \gamma} \abs{f(z)} \int_{a}^{b} \abs{\gamma'(t)} dt
		\end{equation*}
...
\begin{thm}[Cauchy's Theorem for Convex Set]\label{thm:cauchy_s_theorem_for_convex_set}
	...
	\begin{enumerate}
	 	...
	 	\item $\int_{\gamma} f(z) dz = 0$ for any closed path $\gamma \in \Omega$.
	 \end{enumerate} 
\end{thm}
...
\begin{proof}
	...
	Let $a \in \Omega$, and let $[a, z]$ denote the straight line from $a$ to $z$. Since $\Omega$ is a convex set, $[a, z]$ is in $\Omega$. Define $F(z)$\footnote{It can be verified that $F$ is continuous.} $= \int_{[a, z]} f(z) dz$\footnote{This is a keys step: defining an ``antiderivative'' as how we would expect it to be.}.
	...
	\begin{tabular}{L{9cm} C{6cm}}
		...
		{$\!\begin{aligned}
			0 &= \int_{\Delta} f(z) dz \\
			  &= \int_{[a, z]} f(z) dz + \int_{[z, z_0]} f(z) dz +  \int_{[z_0, a]} f(z) dz \\
			  &= F(z) + \int_{[z, z_0]} f(z) dz + \left(- F(z_0) \right)
		\end{aligned}$}
		&
		\resizebox{6cm}{!}{
		\begin{tikzpicture}
	...
	\begin{align*}
		\frac{F(z) - F(z_0)}{z - z_0} - f(z_0)
			&= \frac{1}{z - z_0} \int_{[z_0, z]} f(z) dz - f(z_0) \\
			&= \frac{1}{z - z_0} \int_{[z_0, z]} f(z) - f(z_0) dz \quad \text{since } \int_{[z_0, z]} dz = z - z_0
	\end{align*}
	...
	\begin{equation*}
		\abs{\frac{F(z) - F(z_0)}{z - z_0} - f(z_0)} = \abs{\frac{1}{z - z_0} \int_{[z_0, z]} f(z) - f(z_0) dz} \leq \frac{1}{\abs{z - z_0}} \abs{\int_{[z_0, z]} \epsilon \; dz} = \epsilon
	\end{equation*}
	Hence, by first principle, $F'(z_0) = f(z_0)$. \qed
\end{proof}

\begin{thm}\label{thm:keyhole_thm}
Let $\Omega \subseteq \mathbb{C}$ be a convex open set, and $C$ be a closed circle path in $\Omega$. If $w \in \Omega \setminus C^*$, where $C^*$ is the \hldefn{boundary of $C$}, and $f \in H(\Omega)$, then
	...
	\begin{tabular}{L{10cm} C{5cm}}
		\begin{equation*}
			f(w) \ind{w}{C} = \frac{1}{2 \pi i} \int_{C} \frac{f(z)}{z - w} dz
		\end{equation*}
		where
		\begin{equation*}
			\ind{w}{C} = \begin{cases}
				1 & \text{if } w \text{ is inside } C \\
				0 & \text{if } w \text{ is outside } C
			\end{cases}
		\end{equation*}
		&
		\resizebox{5cm}{!}{
...
\begin{proof}
	...
	We now follow a similar argument as in the proof for \cref{thm:cauchy_s_theorem_for_convex_set}. Let $\epsilon > 0$ such that $\exists \delta > 0$,  so that we can define the ``keyhole'' $\gamma_{\delta, \epsilon}$ which omits $w$. Consider $D(w, \epsilon)$, call the image of the border of $D(w, \epsilon)$ as $C_\epsilon$, let $\delta$ be the width of the ``corridor'', and the two paths that are the ``sides of the corridor'' be called $C_{\delta_1}, C_{\delta_2}$ respectively. Define $G(z) = \int_{[a, z]} g(z) dz$, where $a$ and $z$ are in the interior of $C$ but not in the interior of $C_\epsilon$. Then if we define a set $\Omega'$ such that it contains the interior of $\gamma_{\delta, \epsilon}$, we have that $\Omega'$ is a convex open set, and $G \in H(\Omega')$. By \cref{thm:cauchy_s_theorem_for_convex_set}, $G' = g$.

	Also from \cref{thm:cauchy_s_theorem_for_convex_set}, we have that $\int_{\gamma_{\delta, \epsilon}} g(z) dz = 0$ for any $\epsilon, \delta > 0$. As $\delta \to 0^+$, we have that the integrals over $C_{\delta_1}$ and $C_{\delta_2}$ cancel out. Hence, we are left with

	\begin{equation*}
		\int_{C} g(z) dz + \int_{C_\epsilon} g(z) dz = 0
	\end{equation*}

	Let's put our focus on the smaller circle, $C_\epsilon$. Now as $\epsilon \to 0^+$, $\frac{f(z) - f(w)}{z - w} \to 0$, and thus
	\begin{equation*}
		\int_{C_\epsilon} g(z) dz = \int_{C_\epsilon} \frac{f(z) - f(w)}{z - w} dz \to 0
	\end{equation*}
	Therefore,
	\begin{equation*}
		\int_{C} g(z) dz = 0
	...
	\begin{align*}
		\int_{C} \frac{f(z)}{z - w} dz
			&= \int_{C} \frac{f(w)}{z - w} dz \\
			&= 2 \pi i f(w) \ind{w}{C} \quad \text{where } \ind{w}{C} = \int_{C} \frac{dz}{z - w} 
	\end{align*}
	This completes the proof. \qed

	\hlwarn{Note: sketchy proof; amend on next lecture}
\end{proof}
