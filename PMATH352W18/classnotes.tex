...
\title{PMATH352W18 - Complex Analysis}
\author{Johnson Ng}
\subtitle{Classnotes for Winter 2018}
...
\begin{thm}[Cauchy-Riemann Equations\index{Cauchy-Riemann Equations}]\label{thm:cauchy_riemann_equations}
...
\begin{defn}[Equivalent Parameterization]\label{defn:equivalent_parameterization}
	Let $\gamma_1 : [a, b] \to \mathbb{C}, \gamma_2 : [c, d] \to \mathbb{C}$ where $a, b, c, d \in \mathbb{C}$ describe the path $\gamma^*$. The two parameterizations are said to be \hldefn{equivalent parameterizations} if $\exists h: [a, b] \to [c, d]$ that is a bijection and a continuous function such that
	\begin{equation*}
...
\begin{thm}[Fundamental Theorem of Calculus\index{Fundamental Theorem of Calculus}]\label{thm:fundamental_theorem_of_calculus}
...
\begin{thm}[Goursat's Theorem / Cauchy's Theorem for a triangle\index{Goursat's Theorem}]\label{thm:goursat_s_theorem}
...
\begin{thm}[Cauchy's Theorem for Convex Set\index{Cauchy's Theorem for Convex Set}]\label{thm:cauchy_s_theorem_for_convex_set}
...
\begin{proof}
	...
	Let $a \in \Omega$, and let $[a, z]$ denote the straight line from $a$ to $z$. Since $\Omega$ is a convex set, $[a, z]$ is in $\Omega$. Define $F(z)$\sidenote{It can be verified that $F$ is continuous.} $= \int_{[a, z]} f(z) \dif{z}$\sidenote{This is a keys step: defining an ``antiderivative'' as how we would expect it to be.}.
...
\begin{thm}[Cauchy's Integral Formula 1\index{Cauchy's Integral Formula}]\label{thm:cauchy_s_integral_formula_1}
...
\begin{proof}
	...
	\begin{equation*}
		f(w) = \frac{1}{2 \pi i} \int_{C} \frac{f(z)}{z - w} \dif{z}.
	\end{equation*}
	Let $w_0 \in \Omega$ be the center of $C$. Then $\forall z \in \partial C, 0 < \abs{w - w_0} < \abs{z - w_0}$\sidenote{This is the key step}. This implies that
	\begin{align*}
...
\begin{thm}[Cauchy's Integral Formula 2\index{Cauchy's Integral Formula}]\label{thm:cauchy_s_integral_formula_2}
...
\begin{proof}
	...
	\end{equation*}
	Let $A = \frac{1}{z - w -h}, B = \frac{1}{z - w}$\sidenote{Key step}, then the term in square brackets in \cref{eq:cauchy_s_integral_formula_2_eq1} becomes
	\begin{equation*}
...
\begin{ex}\label{ex:applications_of_cauchy_s_integral_formula}
	\begin{enumerate}
		\item \hlnotea{(Cauchy's Inequality)}\sidenote{In a sense, this inequality implies that as we take higher derivatives, the value of the derivatives become smaller.} Prove that $\forall z_0 \in \mathbb{C} \; \forall R > 0 \in \mathbb{R} \; \forall f \in H(C = D(z_0, R))$ \label{item:cauchy_s_inequality}
			\begin{equation*}
		...
		\item \hlnotea{(Liouville's Theorem)} A bounded entire function $f: \mathbb{C} \to \mathbb{C}$ is a constant\sidenote{The theorem is not true in $\mathbb{R}$, since $\sin x$ is a bounded function differentiable everywhere, but is not a constant.} \sidenote{The theorem also implies that ``trigonometry'' in $\mathbb{C}$ is unbounded, whatever the definition of ``trigonometry'' may be.}.\label{item:liouville_s_theorem}
		...
		\item \hlnotea{(Parseval's Theorem)} Let $\Omega \subseteq \mathbb{C}$ be open, $f \in H(\Omega), \, \bar{D(z_0, R)} \subseteq \Omega$. Then $\forall z \in \bar{D(z_0, R)}, \, f(z) = \sum_{n=0}^{\infty} c_n (z - z_0)^n$, which in turn implies that \sidenote{This is why the $L^2$-norm is perserved, as seen in AMATH231.}\label{item:parseval_s_theorem}
...
\begin{enumerate}
	...
	\begin{proof}
 	...
 	\item \hlnotea{Fundamental Theorem of Algebra}\index{Fundamental Theorem of Algebra} (FTA) Any polynomial $P(z) \in \mathbb{C}[z]$ of degree greater than $1$ has precisely $n$ roots in $\mathbb{C}$, given by $\alpha_1, \alpha_2, ..., \alpha_n$. $P(z)$ can be factored as $P(z) = A(z - \alpha_1)\hdots(z - \alpha_n)$ for some $A \in \mathbb{C}$. \label{item:fta}
...
\begin{thm}[Morera's Theorem\index{Morera's Theorem}]\label{thm:morera_s_theorem}
...
\begin{thm}[Winding Number Theorem\index{Winding Number Theorem}]\label{thm:winding_number_theorem}
...
\begin{proof}
	\begin{enumerate}
		...
		\item Also $\ind{w}{\gamma}$ takes only integer values, thus it must be constant on each open connected component\sidenote{We may invoke \cref{lemma:principle_of_analytic_continuation} but it is, to an extent, unnecessary for such a powerful statement.} (\hlwarn{why?}).
...
\begin{defn}[Removable Singularity, Pole, Essential Singularity]\label{defn:removable_singularity_pole_essential_singularity}
	...
	\begin{enumerate}
		\item $\exists r > 0 \enspace \forall z \in D = D(z_0, r) \enspace \exists g(z) \in H(D) \enspace \forall z \in D \setminus \{z_0\} \enspace g(z) = f(z)$ \\ $\implies f$ has a \hldefn{removable singularity} at $z_0$\sidenote{For the laymen, "the value of $f$ at $z_0$ can be corrected or defined to make it holomorphic in its designated region."}.

		\item $\exists r > 0 \enspace \forall z \in D = D*(z_0, r) \enspace \exists A, B \in H(D) \enspace A(z_0) \neq 0 \, \land \, B(z_0) = 0 \enspace f(z) = \frac{A(z)}{B(z)}$ \\ $\implies f$ has a \hldefn{pole} at $z_0$ (a non-removable singularity)\sidenote{For the laymen, "the singularity of $f$ comes from a zero of its denominator."}
...
\begin{defn}[Zero of Order $n$ \& Simple Zero]\label{defn:zero_of_order_n_&_simple_zero}
	By the above setting, we say that $f$ has a \hldefn{zero of order $n$} at $z_0$.\sidenote{In laymen terms, "Rate at which the function vanishes at $z_0$. The greater $n$ is, the greater the rate."}
...
\begin{defn}[Pole of order $n$ \& Simple Pole]\label{defn:pole_of_order_n_&_simple_pole}
	With the above setting, we say that $f$ has a \hldefn{pole of order $n$} at $z_0$ if the function $B$ has a zero of order $n$\sidenote{In laymen terms, "Rate at which $f$ `grows' near $z_0$."}
...
\begin{defn}[Residue]\label{defn:residue}
	In \cref{thm:theorem_11}, the coefficient $c_{-1}$ is called the \hldefn{residue} of $f$ at the pole $z_0$, denoted $\res{f(z)}{z = z_0}$.
\end{defn}
...
\begin{thm}[Casorati-Weierstrass\index{Casorati-Weierstrass}]\label{thm:casorati_weierstrass}
	...
	\begin{enumerate}
		...
		\item $\forall r > 0, \, B(z_0, r) \subseteq \Omega$ such that $f(B^0(z_0, r))$ is dense in $\mathbb{C}$.\sidenote{$B^0(z_0, r)$ is the punctured ball.}
	\end{enumerate}
\end{thm}
...
\begin{proof}
	...
	\end{gather*}
	Consider $g(z) = \frac{1}{f(z) - w}$\marginnote{Plausible since $f(z) - w \neq 0$.} for $z \in B^0(z_0, r)$, in which $g \in H(B^0(z_0, r))$. Then $\abs{g(z)} \leq \frac{1}{\delta}$ for all $z \in B^0(z_0, r)$, which implies that
	\begin{equation*}
		\lim_{z \to z_0} (z - z_0) g(z) = 0\mathmarginnote{Squeeze Theorem}
	\end{equation*}
	...
	\end{equation*}
	Therefore, $\frac{1}{\tilde{g}} \in H(B(z_0, r))$\sidenote{$\frac{1}{\tilde{g}}$ is the inverse of a non-zero holomorphic function.}
...
\begin{thm}[Cauchy's Residue Theorem\index{Cauchy's Residue Theorem}]\label{thm:cauchy_s_residue_theorem}
...
\section{The Residue Theorem (Continued)} % (fold)
...
\begin{thm}[Cauchy's Residue Theorem - Generalized\index{Cauchy's Residue Theorem - Generalized}]\label{thm:cauchy_s_residue_theorem_generalized}
  ...
  \begin{equation}\label{eq:generalized_residue_theorem}
...
\begin{proof}
  We will first need to prove that $\mathscr{A}$ has only finitely many points.

  Since $f$ is meromorphic, the set of poles, $\mathscr{A}$, must not contain its limit points by definition of meromorphisms, i.e. each of the poles are isolated singularities and $\mathscr{A}^* = \emptyset$. Let $\mathscr{A}' := \{a \in \mathscr{A} \, : \, \ind{a}{\gamma} \neq 0\}$. Suppose, for contradiction, that $\mathscr{A}'$ has infinitely many points. Since the union of $\gamma$ and its interior is compact, by Bolzano-Weierstrass, any infinite subset (subsequence, to be exact, but we may simply index the points in the subset) must converge to a point in the union of $\gamma$ and its interior, call it $w$. Since $\mathscr{A}'$ is a subset (subsequence) of the union of $\gamma$ and its interior, $\mathscr{A}'$ converges to $w$. Now since $\abs{f(z)} \to \infty$ and $z \to a$ for all $a \in \mathscr{A}'$, and since $f$ is continuous since it is meromorphic and hence holomorphic, we have that $\abs{f(z)} \to \infty$ as $z \to w$. Hence $w$ is a pole in the interior of $\gamma$ and is hence in $\mathscr{A}'$. In other words, $\mathscr{A}'$ contains its limit points, which contradicts the fact that $\mathscr{A}' \subseteq \mathscr{A}$ does not contain its limit points. Therefore, there are only finitely many points in $\mathscr{A}'$.

  It remains to prove \cref{eq:generalized_residue_theorem}.

  Since $\mathscr{A}$ is finite, let $\abs{\mathscr{A}} = n$ for some $n \in \mathbb{N}$. For any $a, b \in \mathbb{N}$, $\forall z_a, z_b \in \mathscr{A}, \, \exists r_a, r_b > 0$ such that $z_b \not\in D(z_a, r_a) \, \land \, z_a \not\in D(z_b, r_b)$. Therefore, $\forall l \in \mathbb{N}, \, \forall z_l \in \mathscr{A}$, we can write, $\forall z \in D(z_l, r_l)$, that
  \begin{equation}\label{eq:2_1}
    f(z) = \frac{c_{l, -k_l}}{(z - z_l)^{k_l}} + \hdots + \frac{c_{l, -1}}{z - z_l} + c_{l, 0} + c_{l, 1} (z - z_l) + \hdots,
  \end{equation}
  where $k_l$ is the order of the pole $z_l$. Since we are only concerned with the poles in the interior of $\gamma$, let $\mathscr{A}' := \{a \in \mathscr{A} : \ind{a}{\gamma} = 1 \} = \{z_1, z_2, ..., z_n\}$.

  Consider the function
  \begin{equation*}
    g(z) = \begin{cases}
      f(z) - \displaystyle \sum_{l=1}^{n} \sum_{j=1}^{k_{l}} \frac{c_{l, -j}}{(z - z_l)^j} & \forall z \in \Omega \setminus \mathscr{A}' \\
      c_{l, 0} - \displaystyle \sum_{\substack{m=1 \\ m \neq l}}^{n} \sum_{j=1}^{k_m} \frac{c_{m, -j}}{(z - z_m)^j}       & z \in \mathscr{A}'
    \end{cases}
  \end{equation*}

  At the neighbourhood of each $z_l \in \mathscr{A}'$ save $z_l$ itself, that is in $D(z_l, r_l) \setminus \{z_l\}$ where $D(z_l, r_l)$ is as defined above, we have that
  \begin{equation}\label{eq:2_2}
    g(z) = \underbrace{f(z) - \sum_{j=1}^{k_l} \frac{c_{l, -j}}{(z - z_l)^j}}_{A_{z, l}} - \underbrace{\sum_{\substack{m=1 \\ m \neq l}}^{n} \sum_{j=1}^{k_m} \frac{c_{m, -j}}{(z - z_m)^j}}_{B_{z, l}}
  \end{equation}
  in which we observe that $A_{z, l}$ is holomorphic by Casorati-Weierstrass, and $B_{z, l}$ is holomorphic in $D(z_l, r_l)$ since $\forall z \in D(z_l, r_l) \setminus \{z_l\}$, $z \not\in \mathscr{A}'$ and hence the denominators cannot be zero. Thus $g \in H\big( D(z_l, r_l) \big)$. At $z_l$, $g(z_l)$ agrees with \cref{eq:2_2} by \cref{eq:2_1}. Thus $g \in H\big(D(z_l, r_l)\big)$.

  Then by Cauchy's Theorem,
  \begin{equation*}
    \int_{\gamma} g(z) \dif{z} = 0.
  \end{equation*}
  Then, $\forall z \in \gamma^*$ (image of $\gamma$), since $\mathscr{A} \cap \gamma^* = \emptyset$ (since $\gamma* \in \Omega \setminus \mathscr{A}$), we get
  \begin{equation*}
    \int_{\gamma} f(z) \dif{z} = \int_{\gamma} \sum_{l=1}^{n} \sum_{j=1}^{k_l} \frac{c_{l, -j}}{(z - z_0))^j} \dif{z}
  \end{equation*}

  For each of the terms in $\sum_{j=1}^{k_l} \frac{c_{l, -j}}{(z - z_l)^j}$, for $j \geq 2$, we have that the antiderivative of $\frac{1}{(z - z_l)^j}$ is $- \frac{1}{(j - 1)(z - z_0)^{j - 1}}$. Thus by the Fundamental Theorem of Calculus, since we are integrating over the closed path $\gamma$, the integral of these terms are $0$. When $j = 1$, since $ \frac{1}{2 \pi i} \int_{\gamma} \frac{1}{z - z_l} = \ind{z_l}{\gamma}$, we have that
  \begin{align*}
    \frac{1}{2 \pi i} \int_{\gamma} f(z) \dif{z}
      &= \sum_{l=1}^{n} c_{l, -1} \ind{z_l}{\gamma} \\
      &= \sum_{l=1}^{n} \Big( \res{f(z)}{z = z_l} \Big) \ind{z_l}{\gamma}
  \end{align*}
  as required.

  \noindent Another proof that we can be shown is as follows:

  Using similar notations as above, on each of the neighbourhood of the points in $\mathscr{A}$, we define a keyhole-like path around the points, with a circular path $\gamma_l$, oriented in the opposite direction of $\gamma$, of radius $r_l \geq \epsilon_l > 0$ centered around each $z_l$, connected to the path $\gamma$ through a corridor of width $\delta_l$. This construction is similar to the proof of Cauchy's Integral Formula in our lectures, where we extend the path of $\gamma$ inwards so as to avoid the poles. As each $\delta_l \to 0$, we have
  \begin{equation*}
    \int_{\gamma} f(z) \dif{z} = \sum_{l=1}^{n} \int_{\gamma_l} f(z) \dif{z} = \sum_{l=1}^{n} \int_{\gamma_l} \sum_{j=-k_l}^{\infty} c_{l, j} (z - z_l)^j \dif{z} 
  \end{equation*}
  Now for $-k_l \leq j \leq 2$, the term $c_{l, j} (z - z_0)^j$ have antiderivatives as described above, and are thus $0$ by FTC, on the path $\gamma_l$. When $j \geq 0$, $\sum_{j=0}^{\infty} c_{l, j} (z - z_l)^j$ is a holomorphic function by construction, and hence their integrals are $0$ by Cauchy's Theorem. We are, therefore, left with
  \begin{equation*}
    \int_{\gamma} f(z) = \sum_{l=1}^{n} \int_{\gamma_l} \frac{c_{l, -1}}{z - z_l} \dif{z}  
  \end{equation*}
  which in turn implies
  \begin{equation*}
    \frac{1}{2 \pi i} \int_{\gamma} f(z) \dif{z} = \sum_{l=1}^{n} \Big( \res{f(z)}{z = z_l} \Big) \ind{z_l}{\gamma_l}
  \end{equation*}
  This implies the desired result (through a similar proof given in \cref{thm:cauchy_s_residue_theorem}). \qed
\end{proof}
...
% section the_residue_theorem_continued (end)

\section{Applications of Cauchy's Residue Theorem} % (fold)
...
\begin{ex}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item use symmetry, decay of function, etc., in the limit (\hlwarn{we shall see more about this later on})
    \end{itemize}
  \end{enumerate}
...
% section applications_of_cauchy_s_residue_theorem (end)

% chapter lecture_23_mar_12th_2018 (end)

\chapter{Lecture 24 Mar 14 2018}
  \label{chapter:lecture_24_mar_14_2018}

\section{Application of Cauchy's Residue Theorem (Continued)} % (fold)
...
\begin{ex}
  Show that $\int_{-\infty}^{\infty} \frac{\sin x}{x} \dif{x} = \pi$ \\
  (Note: This integrand is not absolutely convergent)

  If we try $f(z) = \frac{\sin z}{z}$ on some semi-circle arc $C_R$ with $\abs{f(z)} \leq \frac{1}{R}$, then
  \begin{equation*}
    \abs{\int_{C_R} f(z) \dif{z} } \leq \int_{C_R} \frac{1}{R} \abs{\dif{z}} = \frac{\text{length of } C_R}{R} \approx \pi
  \end{equation*}
  which means that the \hlnotea{decay} of the $f$ is insufficient to help us compute our desired result.

  Consider $\sin x = \frac{e^{ix} - e^{-ix}}{2 i}$. We then need to show
  \begin{equation*}
    I = \int_{-\infty}^{\infty} \frac{e^{ix} - e^{-ix}}{2 i x} \dif{x} = \pi
  \end{equation*}
  Let $f(z) = \frac{e^{iz}}{z} = \frac{1}{z} (1 + iz + \frac{(iz)^2}{2} + \hdots )$. Thus $F$ has a simple poles at $z = 0$ with residue $1$.

  Consider the countour $\Gamma_R$:
  \begin{figure}
    \begin{center}
      \resizebox{6cm}{!}{
        \begin{tikzpicture}
          \draw[->] (-5.5, 0) -- (5.5, 0) node[right] {$\re$};
          \draw[->] (0, -0.5) -- (0, 6) node[above] {$\im$};
          \draw[midarrow={latex}{0.7}] (5, 0) arc(0:180:5) node [midway,above,right,xshift=5,yshift=7] {$C_R$};
          \draw[midarrow={latex}{0.7}] (-3, 0) arc(-180:-360:3) node [midway,above,left,xshift=-5,yshift=7] {$C_\epsilon$};
          \draw[midarrow={latex}{0.5}] (-5,0) -- (-3,0);
          \draw[midarrow={latex}{0.5}] (3, 0) -- ( 5,0);
          \node[label={270:{$R$}},circle,fill,inner sep=2pt] at (5,0) {};
          \node[label={270:{$-R$}},circle,fill,inner sep=2pt] at (-5,0) {};
          \node[label={270:{$\epsilon$}},circle,fill,inner sep=2pt] at (3,0) {};
          \node[label={270:{$-\epsilon$}},circle,fill,inner sep=2pt] at (-3,0) {};
        \end{tikzpicture}
      }
    \end{center}
    \caption[loftitle]{Contour $\Gamma_R$}
  \end{figure}
  ...
  Using a similar argument\marginnote{Work out the similar case as an exercise
  \begin{ex}
    Show that
    \begin{equation*}
      \int_{-\infty}^{\infty} \frac{e^{- i x}}{x} \dif{x} = - \pi i
    \end{equation*}
  \end{ex}}, it can be shown that
  \begin{equation*}
...
Refer to Stein \& Shakarchi, Section 2.1, for \hlimpo{more examples}.

\newthought{Interesting Examples} from Stein \& Shakarchi
\begin{eg}
  Prove that for $0 < a < 1$,
  \begin{equation*}
    \int_{-\infty}^{\infty} \frac{e^{ax}}{1 + e^x} \dif{x} = \frac{\pi}{\sin \pi a} 
  \end{equation*}

  Choose $f(z) = \frac{e^{az}}{1 + e^z}$. Note that $f$ has one pole at $z = i \pi$. Consider a rectangle lying on the upper-half side of the plane with its base lying on the real axis and its top the line $2 \pi$.
    ...
    \begin{center}
      \resizebox{6cm}{!}{
        \begin{tikzpicture}
          \draw[->] (-5.5, 0) -- (5.5, 0) node[right] {$\re$};
          \draw[->] (0, -0.5) -- (0, 6) node[above] {$\im$};
          \draw[midarrow] (-5,5) -- (-5, 0);
          \draw[midarrow] (-5,0) -- (0,0);
          \draw[midarrow] (0,0) -- (5,0);
          \draw[midarrow] (5,0) -- (5,5);
          \draw[midarrow] (5,5) -- (0,5);
          \draw[midarrow] (0,5) -- (-5,5);
          \node[label={135:{$2 \pi$}},circle,fill,inner sep=2pt] at (0,5) {};
          \node[label={135:{$\pi$}},circle,fill,inner sep=2pt] at (0,2.5) {};
          \node[label={270:{$R$}},circle,fill,inner sep=2pt] at (5,0) {};
          \node[label={270:{$-R$}},circle,fill,inner sep=2pt] at (-5,0) {};
          \node[label={90:{$\Gamma_R$}}] at (3,5) {};
        \end{tikzpicture}
      }
    \end{center}
    \caption[loftitle]{Contour $\Gamma_R$}
  ...
  \begin{equation*}
    \frac{1}{2 \pi i} \int_{\Gamma_R} f(z) \dif{z} = \res{f(z)}{z = i \pi}
  \end{equation*}
  To evaluate the residue, note that
  \begin{equation*}
    (z - i \pi) \cdot \frac{e^{az}}{1 + e^z} = e^{az} \cdot \frac{z - i \pi}{e^z - e^{i \pi}}
  \end{equation*}
  and $\lim_{z \to i \pi} \frac{z - i \pi}{e^z - e^{i \pi}}$ is the inverse of the difference quotient (of first principles), and is hence $e^{i \pi} = -1$. Therefore
  \begin{equation*}
    \res{f(z)}{z = i \pi} = \lim_{z \to \infty} (z - i \pi) \cdot \frac{e^{az}}{1 + e^z} = - \lim_{z \to i \pi} e^{az} = - e^{a i \pi}.
  \end{equation*}

  Let $I$ be the desired integral and 
  \begin{equation*}
     I_R := \int_{-R}^{R} f(z) \dif{z}
  \end{equation*}
  so that we may have $I_R \to I$ as $R \to \infty$. This will describe the bottom part of the rectangle. For the top part of the integral, notice that it is simply
  \begin{align*}
    \int_{R}^{-R} \frac{e^{az}}{1 + e^z} \dif{z}
      &= - \int_{-R}^{R} \frac{e^{a (x + 2 \pi i)}}{1 + e^{x + 2 \pi i}} \dif{x} \\
      &= - \int_{-R}^{R} \frac{e^{ax} e^{2 \pi i a}}{1 + e^x e^{2 \pi i}} \dif{x} \\
      &= - e^{2 \pi i a} \int_{-R}^{R} \frac{e^{ax}}{1 + e^x} \dif{x} \enspace \because e^{2 \pi i} = 1 \\
      &= - e^{2 \pi i a} I_R.
  \end{align*}

  Let $\gamma_r$ parameterize the vertical path on the right side of the rectangle, i.e. $\gamma_r : [0, 2 \pi] \to \mathbb{C}$ such that $\gamma_r (t) = R + it$. Then
  \begin{align*}
    \abs{\int_{\gamma_r} \frac{e^{az}}{1 + e^z} \dif{z}}
      &= \abs{\int_{0}^{2 \pi} \frac{e^{a(R + ti)}}{1 + e^{R + ti}} \dif{t}} \\
      &\leq \frac{e^{aR}}{1 + e^R} \int_{0}^{2 \pi} \dif{t}
  \end{align*}
  which goes to $0$ as $R \to \infty$ since $aR < R$ for $0 < a < 1$. Thus the integral of the vertical path evaluates to $0$. We can perform a similar procedure for the path on the left and end up with the same result.

  With that, we have that
  \begin{equation*}
    I_R - e^{2 \pi i a} I_R = - 2 \pi i e^{\pi i a}
  \end{equation*}
  and so
  \begin{equation*}
    I_R = \frac{- 2 \pi i e^{\pi i a}}{1 - e^{2 \pi i a}} = \frac{2 \pi i}{e^{\pi i a} - e^{- \pi i a}} = \frac{\pi}{\sin \pi a}  
  \end{equation*}
  and hence as $R \to \infty$, we have
  \begin{equation*}
    \int_{-\infty}^{\infty} \frac{e^{ax}}{1 + e^x} \dif{x} = \frac{\pi}{\sin \pi a} 
  \end{equation*}
  as required.
...
% section application_of_cauchy_s_residue_theorem_continued (end)

% chapter lecture_24_mar_14_2018 (end)
...
\end{equation*}
can be interpreted as the change in the argument\index{argument} of $f$ as $z$ traverses the curve $\gamma$. Moreover, assuming that $\gamma$ is a closed path, this change of argument is determined entirely by the zeros and poles of $f$ in $\gamma$. 

\begin{note}[Stein \& Shakarchi, pg. 89]
...
\begin{thm}[Argument Principle\index{Argument Principle}]\label{thm:argument_principle}
...
\begin{proof}
  (\hlwarn{include proof here: use \cref{thm:cauchy_s_residue_theorem_generalized} to CTP}).
\end{proof}
...
\begin{thm}[Rouch\'e's Theorem\index{Rouch\'e's Theorem}]\label{thm:rouche_s_theorem}
...
% chapter lecture_25_mar_16_2018 (end)

\chapter{Lecture 26 Mar 19 2018}
  \label{chapter:lecture_26_mar_19_2018}

\section{The Argument Principle (Continued)} % (fold)
...
\begin{ex}
  ...
  \begin{solution}
    Let $\gamma$ be the circle $\abs{z} = 1$, oriented anticlockwise. Let $g(z) = P(z), \, f(z) = -5z^3$\sidenote{We pick the dominant term in $P$ for $f$}. Then $\abs{f(z)} = \abs{5z^3} = 5$, and
    \begin{align*}
...
\hlimpo{You should try more of these problems from the recommended texts.}

\subsection{Alternative Proof for FTA} % (fold)
...
\begin{defn}[Monic Polynomial\index{Monic Polynomial}]\label{defn:monic_polynomial}
  A monic polynomial is a polynomial with a leading coefficient of $1$.
\end{defn}

\begin{defn}[Monomial\index{Monomial}]\label{defn:monomial}
  A monomial is a polynomial with only one term.
\end{defn}

Recall the statement of the Fundamental Theorem of Algebra\index{Fundamental Theorem of Algebra} (FTA)
...
\begin{proof}
  ...
  \end{equation*}
  with $a_1 \in \mathbb{C}$ for $i \in [1, n - 1] \subset \mathbb{N}$. Let $\gamma$ be the circle $\abs{z} = R > \max\left\{\sum_{j=0}^{n-1} \abs{a_j}, 1 \right\}$\sidenote{This is chosen from the later part of the proof}, oriented anticlockwise. Let $f(z) = z^n$. Then $\abs{f(z)} = R^n$ on $\gamma$. We also have
  \begin{align*}
    \abs{g(z) - f(z)}
  ...
  \textit{\hlnoteb{Exercise: }} Show that these are the only zeros of $g(z)$, using factorization of polynomials in the ring $\mathbb{C}[z]$.
...
% subsection alternative_proof_for_fta (end)

\subsection{Open Mapping Theorem} % (fold)
\label{sub:open_mapping_theorem}

\begin{thm}[Open Mapping Theorem\index{Open Mapping Theorem}]\label{thm:open_mapping_theorem}
...
\begin{proof}
  ...
  \hlnotec{WTS} $w_0 \in f\big(B(z_0, \delta) \big)^0$.
  ...
  \hlnotec{Want to have} $G(z)$ having a zero inside $\gamma$ for $w$ ``close enough'' to $w_0$.
...
% section the_argument_principle_continued (end)

% chapter lecture_26_mar_19_2018 (end)

\chapter{Lecture 27 Mar 21 2018}
  \label{chapter:lecture_27_mar_21_2018}

\section{Introductory Passage to Log Functions in \texorpdfstring{$\mathbb{C}$}{C}} % (fold)
...
We shall try to approach the problem via \hldefn{analytic continuation}.

\begin{ex}[A simple problem in analytic continuation]
  Let $f(z) = \sum_{n=0}^{\infty} z^n$ for $\abs{z} < 1$. We want to \hlnotea{analytically continue} $f$ onto $\mathbb{C}$ if possible.
...
In real analysis, $\log$ is the inverse of $e$\sidenote{$e$ in $\mathbb{R}$ is 1-1, and goes from $\mathbb{R} \to \mathbb{R}^+$}. But on $\mathbb{C}$, the exponential function is not 1-1, e.g.
\begin{equation*}
  e^z = 1 \iff z \in 2 \pi i \mathbb{Z}
\end{equation*}

As such, we would like to restrict the domain (\hlwarn{why?}) for the exponential function. That begs the question: what is the natural domain on which $\log z$ lives for $z \in \mathbb{C}$?

\begin{itemize}
  \item Globally, we would require the notion of \hlnotea{Riemann Surfaces}
  \item Locally, we would require the notion of \hlnotea{Simply Connected Domains}
\end{itemize}

(\hlwarn{What does local and global mean here?})

% section introductory_passage_to_log_functions_in_c (end)

\section{Simply Connected Domains} % (fold)
\label{sec:simply_connected_domains}

\begin{defn}[Homotopy\index{Homotopy} (Poincar\'e)]\label{defn:homotopy}
  Let $X$ be a topological space\sidenote{which we did not define}. Recall that a curve in $X$ is a continuous map $\gamma : I \to X$ where $I = [0, 1]$, and $\gamma$ is said to be closed if $\gamma(0) = \gamma(1)$.

  Two closed curves $\gamma_0$ and $\gamma_1$ are said to be \hldefn{homotopic} if $\exists H : I \times I \to X$ with
  \begin{equation*}
    H(s, 0) = \gamma_0(s) \enspace H(s, 1) = \gamma_1(s)
  \end{equation*}
  and $H(s, t)$ be continuous with respect to $s$ and $t$.

  \textbf{Alternative Definition from Stein-Shakarchi - Complex Analysis}\sidenote{I preferred this definition cause it's easier to read, but I shall be using the definition from the lecture for the class itself unless stated otherwise}
  ...
  \end{equation*}
  where $\alpha, \beta \in \Omega$. The two curves are said to be \hldefn{homotopic} in $\Omega$ if for each $0 \leq s \leq 1$, $\exists \gamma_s \subset \Omega$ parameterized by $\gamma_s(t)$ defined on $[a, b]$, such that $\forall s$, 
  \begin{equation*}
...
Loosely speaking, $\gamma_0, \gamma_1$ are homotopic if we can \hlnoteb{continuously deform} $\gamma_0$ to $\gamma_1$ (wlog) without any obstruction in $X$.

\begin{defn}[Simply Connected Domain\index{Simply Connected Domain}]\label{defn:simply_connected_domain}
  Let $\Omega \subseteq \mathbb{C}$ be open. We say $\Omega$ is \hldefn{simply connected} if $\Omega$ is connected, and $\forall \gamma$ that is closed in $\Omega$ is homotopic to a point (i.e. a constant map $\gamma : I \to X$).
\end{defn}
...
\begin{note}
  ...
  i.e. in a simply connected domain, the \hlnoteb{integral does not depend on the chosen path}\sidenote{This should remind you of \hyperref[thm:goursat_s_theorem]{Goursat's Theorem} and \hyperref[thm:cauchy_s_theorem_for_convex_set]{Cauchy's Theorem}}.
\end{note}

% section simply_connected_domains (end)

% chapter lecture_27_mar_21_2018 (end)

\chapter{Lecture 28 Mar 23 2018}
  \label{chapter:lecture_28_mar_23_2018}

\section{Constructing Logarithm} % (fold)
\label{sec:constructing_logarithm}

\begin{thm}[Theorem 17]\label{thm:theorem_17}
  Suppose $\Omega$ is simply connected with $0 \not\in \Omega$. Then in $\Omega$, we can define a function, call it $\Log z$\sidenote{This is called a branch of the logarithm}, such that
  \begin{enumerate}
...
\begin{proof}
  \begin{enumerate}
    ...
    \begin{equation}\tag{$\dagger$}\label{eq:first_defn_of_log}
      \Log z = w_0 + \int_{z_0}^{z} \frac{1}{w} \dif{w}
    \end{equation}
...
% section constructing_logarithm (end)

\section{Branches of the Logarithm} % (fold)
\label{sec:branches_of_the_logarithm}

\begin{enumerate}
  \item (\hldefn{Principal Branch}) Let $\Omega_1 = \mathbb{C} \setminus (- \infty, 0]$. We will write $z \in \Omega_1$ as $z = re^{i \theta}$ with $r > 0$ and $\theta \in (- \pi, \pi)$. Pick $z_0 = 1 \, \land \, w_0 = 0$> By \cref{eq:first_defn_of_log}, $\Log z = \int_{1}^{z} \frac{1}{w} \dif{w}$. Then in this case,
  \begin{equation*}
  ...
  \begin{center}
      \begin{tikzpicture}
        ...
        \node[label={315:{$0$}},circle,draw,inner sep=2pt] at (0,0) {};
        \foreach \x in {-4.5, -4, ..., -0.5}
          \draw (\x cm - 2pt, -2pt) -- (\x cm + 2pt, 2pt);

        \coordinate (a) at (1, 0);
        \coordinate (r) at (3.5,0);
        \coordinate (z) at (2.5, 2.44948974278317809819);
        \node[label={270:{$1$}},circle,fill,inner sep=1pt] at (a) {};
        \node[label={270:{$r$}},circle,fill,inner sep=1pt] at (r) {};
        \node[label={270:{$z$}},circle,fill,inner sep=1pt] at (z) {};
        \draw[midarrow] (a) .. controls (2,0.5) and (1.5,2) .. (z);
        \draw[midarrow] (a) -- (r);
        \draw[midarrow] (r) arc (0:45:3.5);
      \end{tikzpicture}
  \end{center}
  Then
  \begin{align*}
...
\begin{enumerate}
  \setcounter{enumi}{1}
  \item (a different branch) Let $\Omega_2 = \mathbb{C} \setminus [0, \infty)$. Write $z \in \Omega_2$ as $z = re^{i \theta}$ with $r > 0 \, \land \, \theta \in (0, 2 \pi)$. Now we can pick \hlnoteb{some function} so that
  \begin{equation*}
...
% section branches_of_the_logarithm (end)

% chapter lecture_28_mar_23_2018 (end)

\chapter{Lecture 29 Mar 26 2018}
  \label{chapter:lecture_29_mar_26_2018}

\section{Examples for Analytic Continuation} % (fold)
\label{sec:examples_for_analytic_continuation}

\hldefn{Gamma Function}
...
\begin{note}
  \begin{enumerate}
    ...
    \item $\Gamma$ is a \hldefn{Functional Equation}: We can repeat Euler's calculation to show that
    \begin{equation*}
...
% section examples_for_analytic_continuation (end)

\section{Characterizing Logarithms} % (fold)
...
% section characterizing_logarithms (end)

% chapter lecture_29_mar_26_2018 (end)
...
\begin{proof}
  ...
  \end{equation*}
  where $\{c_j\}_{j \in \mathbb{Z}_{\geq 0}} \subseteq \mathbb{C}$\sidenote{$g$ can be obtained by term-wise integration of the Taylor series for $\frac{f'}{f}$}.
  ...
  $\therefore \forall z \in \mathbb{C} \enspace F'(z) \equiv 0$. Now because of that and $F \in H(\mathbb{C})$, $\exists A \in \mathbb{C} \enspace \forall z \in \mathbb{C} \enspace F(z) \equiv A$\sidenote{By considering the Taylor series for $F$}.
...
The construction of $h$ motivates us to study our next topic: \hldefn{infinite products}.
...
\begin{defn}[Infinite Products\index{Infinite Products}]\label{defn:infinite_products}
  Let $u_1, u_2, ...$ be a sequence in $\mathbb{C}$. Let
  \begin{equation*}
    P_N = \prod_{j=1}^{N} \left( 1 + u_j \right)
  \end{equation*}
  be the $N^{\text{th}}$ \hldefn{partial product}. If $\lim_{N \to \infty} P_N$ exists, then we say that the \hldefn{infinite product}, $\prod_{j=1}^{\infty} \left(1 + u_j \right)$, converges, and write
  \begin{equation*}
...
\begin{lemma}[Bounds of the Partial Product]\label{lemma:bounds_of_the_partial_product}
  ...
  \begin{enumerate}
    ...
    \item $\forall N \in \mathbb{N} \enspace \abs{P_N - 1} \leq P_N^* - 1$\sidenote{Note that $P_N^* \geq 1$}
  \end{enumerate}
\end{lemma}
...
\begin{proof}
  ...
  \WTS $\{P_N\}$ is convergent. It suffices to show that $\{P_N\}$ is Cauchy, i.e.\sidenote{Cauchy convergence from real analysis}
  \begin{equation*}
  ...
  \begin{align*}
    \abs{P_M - P_N} &= \abs{P_N \left( \prod_{j = N + 1}^{M} (1 + u_j) - 1\right)} \\
      ...
      &\leq P_N^* \left(\exp{\left(\sum_{j=N + 1}^{M} \abs{u_j} \right)} - 1 \right).
  \end{align*}
  ...
  \end{equation*}
  \footnotetext{Since it is convergent and hence Cauchy.}So by choosing the appropriate $N_0$ we can have
  \begin{equation*}
...
% chapter lecture_30_mar_28_2018 (end)

\chapter{Lecture 31 Apr 02 2018}
  \label{chapter:lecture_31_apr_02_2018}

\section{Infinite Products (Continued)} % (fold)
...
\begin{remark}
  To apply \cref{thm:theorem_19} to a sequence of functions $\{u_n(z)\}$ in some region $\Omega \subseteq \mathbb{C}$, we need $\sum u_n(z)$ to converge absolutely and uniformly\sidenote{No dependence on $z$, which is part of the definition of \hldefn{uniform convergence}.}.
\end{remark}

\subsection{Application to Riemann Zeta Function} % (fold)
\label{sub:application_to_riemann_zeta_function}

We define $\zeta (s) = \sum_{n=1}^{\infty} \frac{1}{n^s}$ for $\re{(s)} > 1.$ This function is the well-known \hldefn{Riemann Zeta Function}

\begin{remark}
  \begin{enumerate}
    \item The series $\sum_{n=1}^{\infty} \frac{1}{n^s}$ is absolutely convergent for $\re{(s)} > 1$.
    \item By the construction of the function, it is holomorphic/analytic for $\re{(s)} > 1$\sidenote{Requires the Weierstrass' M-test.}
  \end{enumerate}
\end{remark}

\newthought{(History)} Euler looked at the series with real numbers first. It was not until Riemann extended the function to become a function with complex variables that the series became well-known, and hence Riemann's name is prepended to the function instead of Euler.

\newthought{The series} can be analytically continued to the entire complex plane (using the functional equation\sidenote{This is similar to what we did for the Gamma function.}), except for a simple pole at $s = 1$, i.e.
\begin{equation*}
  \lim_{s \to 1^+} (1 - s) \zeta (s) = 1.\footnotemark
\end{equation*}
\footnotetext{\hyperref[thm:cauchy_s_residue_theorem]{Cauchy's Residue Theorem}}

\newthought{Euler showed} that for $\re{(s)} > 1$, $\sum_{n=1}^{\infty} \frac{1}{n^s} = \prod_{p \text{prime}} (1 + \frac{1}{p^s} + \frac{1}{p^{2s}} + \hdots)$. Observe that $RHS$ converges absolutely for $\re{(s)} > 1$. This identity is known as \hldefn{Euler's Identity} and it is simply a statement about the unique factorization of integers into primes\sidenote{This is the \hldefn{Fundamental Theorem of Arithmetic}}.
...
\begin{crly}[Corollary for Theorem 19]\label{crly:corollary_for_theorem_19}
  $\zeta(s) \neq 0$ for $\re{(s)} > 1$.
\end{crly}
...
\begin{proof}
  ...
  For each $s$, we have that each of the sums $\frac{1}{p^s} + \frac{1}{p^{2s}} + \hdots$ converges absolutely for $\re{(s)} > 1$. Also, $\sum_{n=1}^{\infty} u_n$ converges absolutely and uniformly for $\re{(s)} > 1$.\sidenote{These two statements are not too hard to make reliable heuristics to make sense that they are true.}
...
% subsection application_to_riemann_zeta_function (end)

% section infinite_products_continued (end)

% chapter lecture_31_apr_02_2018 (end)

\chapter{Lecture 32 Apr 04 2018}
  \label{chapter:lecture_32_apr_04_2018}

\section{Infinite Products (Continued 2)} % (fold)
\label{sec:infinite_products_continued_2}

\subsection{Weierstrass Products} % (fold)
\label{sub:weierstrass_products}

\newthought{Problem: } Given a sequence $\{a_n\}_{n \in \mathbb{N}} \subseteq \mathbb{C}$, where $\forall n \in \mathbb{N}, \, a_n \neq 0$, construct an entire function $f$ with zeros precisely at each of the $a_n$'s.

It is tempting to consider an infinite product such as $\prod_{n=1}^{\infty} \left(1 - \frac{z}{a_n}\right)$. But this may not converge all the time except for some specific sequence $\{a_n\}$.

The idea to approach this problem is to take the product of exponential factors so that it takes care of the convergence problem.

\newthought{Define: } $\forall n \in \mathbb{N}$
\begin{align*}
  P_n(z) &:= z + \frac{z^2}{2} + \frac{z^3}{3} + \hdots + \frac{z^n}{n} \\
  E_n(z) &:= (1 - z) \exp{\left( P_n(z) \right)}
\end{align*}
and for $n = 0$, define
\begin{equation*}
  E_0(z) := 1 - z
...
\newthought{Observe } that
\begin{itemize}
  \item $E_n$ is entire and has a simple zero at exactly $z = 1$.
  \item $E_n(0) = 1$ and $E_n(1) = 0$
  \item $P_n'(z) = 1 + z + z^2 + \hdots + z^{n - 1} = \frac{z^n - 1}{z - 1}$\sidenote{Finite geometric sum}
\end{itemize}

\begin{thm}[Theorem 20]\label{thm:theorem_20}
...
\begin{proof}
  Note that
  \begin{align*}
    E_n'(z) &= (1 - z) \exp{\left( P_n(z) \right)} \cdot \left(\frac{z^n - 1}{z - 1}\right) - \exp{\left( P_n(z) \right)} \\
      &= - z^n \exp{( P_n(z) )}
  \end{align*}
  and note that by the Taylor expansion of the exponent,
  \begin{equation*}
    \exp{( P_n(z) )} = 1 + P_n(z) + \frac{( P_n(z) )^2}{2!} + \hdots.
  \end{equation*}
  Since the power/Taylor expansion of $P_n(z)$ has non-negative and real coefficients, so does $\exp{( P_n(z) )}$. Therefore, we have
  \begin{align}
    -E_n'(z) &= z^n \exp{( P_n(z) )} \nonumber \\
      &= z^n \cdot \sum_{m=0}^{\infty} a_m z^m \enspace a_0 \neq 0\footnotemark, \, \forall m \in \mathbb{N} \; a_m \geq 0 \nonumber \\
      &= \sum_{m=n}^{\infty} c_m z^{m} \enspace c_0 \neq 0, \, \forall m > n \; c_m \geq 0 \tag{$\star$}\label{eq:thm20_eq1},
  \end{align}
  \footnotetext{This is because the exponential function is non-zero.}
  which holds $\forall z \in \mathbb{C}$ since $E_n'$ is entire\sidenote{This is true by \cref{thm:convergence_in_the_radius_of_convergence}, since $E_n$ is entire.}.

  Now note that
  \begin{align*}
    \int_{0}^{z} - E_n'(w) \dif{w} &= - E_n(z) + E_n (0) \enspace \text{by FTC} \\
      &= 1 - E_n(z).
  \end{align*}
  Also, by \cref{eq:thm20_eq1},
  \begin{equation*}
    \int_{0}^{z} - E_n'(w) \dif{w} = \sum_{m=n}^{\infty} \frac{c_m}{m + 1} z^{m + 1}.
  \end{equation*}
  Therefore,
  \begin{equation*}
    1 - E_n(z) = \sum_{m=n}^{\infty} \frac{c_m}{m + 1} z^{m + 1} = \frac{c_n}{n + 1} z^{n + 1} + \hdots,
  \end{equation*}
  which implies that
  \begin{equation*}
    \frac{1 - E_n(z)}{z^{n + 1}} = \sum_{m=n}^{\infty} \frac{c_m}{m + 1} z^{m - n} \quad \because c_n \neq 0
  \end{equation*}

  Let $\phi_n(z) := \frac{1 - E_n(z)}{z^{n + 1}} = \sum_{k=0}^{\infty} b_k z^k$, where as defined, we have that $b_0 \neq 0$ and $\forall k \in \mathbb{N} \; b_k \geq 0$. By the Triangle Inequality, and since $\abs{z} \leq 1$, we have that
  \begin{align*}
    \abs{\frac{1 - E_n(z)}{z^{n + 1}} } &= \abs{\sum_{k=0}^{\infty} b_k z^k}
      \leq \sum_{k=0}^{\infty} \abs{b_k z^k}
      \leq \sum_{k=0}^{\infty} \left( \abs{b_k} \cdot 1 \right) \\
      &= \sum_{k=0}^{\infty} b_k = \phi_n (1) = \frac{1 - E_n(1)}{1} = 1.
  \end{align*}
  Hence for $\abs{z} \leq 1$, we have the desired inequality,
  \begin{equation*}
    \abs{1 - E_n(z)} \leq \abs{z}^{n + 1}
  \end{equation*}\qed
\end{proof}

\begin{thm}[Theorem 21]\label{thm:theorem_21}
  Let $\{a_n\}_{n \in \mathbb{N}} \subseteq \mathbb{C}$ with $a_n \to \infty$ as $n \to \infty$, and $\forall n \in \mathbb{N}, \, a_n \neq 0$. Then the \hldefn{Weierstrass Product}
  \begin{equation}\label{eq:weierstrass_product}
    f(z) = \prod_{n=1}^{\infty} E_n \left( \frac{z}{a_n} \right)
  \end{equation}
  is entire and has zeros at exactly $\{a_n\}$ and nowhere else.
\end{thm}

\begin{proof}
  Let $R > 0$ and $\Omega = D(0, R) \subseteq \mathbb{C}$. Let $z \in \Omega$. $\because \abs{a_n} \to \infty$, $\exists N_0 > 0 \enspace \forall n \geq N_0 \enspace \abs{a_n} \geq 2R$. Note
  \begin{equation*}
    \prod_{n=N_0}^{\infty} E_n\left( \frac{z}{a_n} \right) = \prod_{n=N_0}^{\infty} \left(1 + \underbrace{\left(E_n \left( \frac{z}{a_n} \right) - 1 \right)}_{\text{call this } u_n(z)} \right)
  \end{equation*}

  On $\Omega$, since $\abs{z} \leq R \, \land \, \forall n \geq N_0 \enspace \abs{a_n} \geq 2R$, by \cref{thm:theorem_20} we have that $\forall n \geq N_0 \, \land \, \forall z \in \Omega$,
  \begin{equation*}
    \abs{\frac{z}{a_n}} \leq 1 \implies \abs{E_n\left( \frac{z}{a_n} \right) - 1} \leq \abs{\frac{z}{a_n} }^{n + 1} \leq \frac{1}{2^{n + 1}} 
  \end{equation*}
  Then by \cref{thm:theorem_19} and its remark, since $\sum_{n=1}^{\infty} u_n(z)$ converges absolutely and uniformly on $\Omega$, the infinite product $\prod_{n=N_0}^{\infty} \left(1 + u_n(z) \right)$ converges. \\
  \hlnotec{Exercise} To prove that the infinite product converges uniformly on $\Omega$, it suffices to show that it satisfies the Cauchy criterion: Define $f_n := \sum_{j=n}^{\infty} (1 + u_j(z))$. Note that for $m > n \geq N_0$, 
  \begin{align*}
    \abs{f_m -  f_n} &= \abs{\sum_{j=n}^{m - 1} \big( 1 + u_j(z) \big)} \\
      &\leq \abs{\sum_{j=n}^{m-1} \big( 1 + \abs{u_j(z)} \big)} \enspace \text{by 2 of } \cref{lemma:bounds_of_the_partial_product} \\
      &\leq \exp{\sum_{j=n}^{m-1} \abs{u_j(z)}} \enspace \text{by 1 of } \cref{lemma:bounds_of_the_partial_product}
  \end{align*}
  So $\forall \epsilon > 0$, since $\sum_{j=1}^{\infty} \abs{u_n(z)}$ converges, $\exists N_0 > 0$\sidenote{choose an $N_0$ different from the earlier $N_0$ if necessary}$\enspace \forall m > n \geq N_0$, such that
  \begin{equation*}
    \sum_{j = n}^{m - 1} \abs{u_n(z)} \leq \epsilon
  \end{equation*}
  and hence
  \begin{equation*}
    \abs{f_m - f_n} \leq e^\epsilon,
  \end{equation*}
  thus satisfying the Cauchy criterion and hence showing that the infinite product is uniformly convergent.

  \hlnotec{Exercise}\marginnote{A more complete statement and proof of this statement is available in Stein \& Shakarchi's Complex Analysis (Chapter 2, Section 5.2, Theorem 5.2).} We now need to show that the infinite product is a holomorphic function on $\Omega$. Note that by construction of $E_n$, it has no poles, and is entire. Note that we have that the $\{f_n\}_{n \in \mathbb{N} }$ is a sequence in $\mathbb{C}$ that is uniformly convergent to the function $f(z) = \prod_{n=1}^{\infty} E_n \left( \frac{z}{a_n} \right)$ in $\Omega$. Let $D$ be any disc whose closure\sidenote{$\bar{D} := \{ K \in \mathbb{C} : K$ closed $\, \land \, D \subseteq K \}$} is contained in $\Omega$, and $T$ any triangle in $D$. Then by \hyperref[thm:goursat_s_theorem]{Goursat's Theorem},
  \begin{equation*}
    \int_{T} f_n = 0.
  \end{equation*}
  Since $\{f_n\}_{n \in \mathbb{N}}$ converges uniformly to $f$, $f$ is continuous\sidenote{This is a relatively easy proof that can be done using techniques from Real Analysis.
  \begin{ex}
    Prove that $f$ is continuous under the supposition that $\{f_n\}_{n \in \mathbb{N}}$ is uniformly convergent.
  \end{ex}
  } and
  \begin{equation*}
    \int_{T} f_n \to \int_{T} f.
  \end{equation*}
  Thus $\int_{T} f = 0$. Then by \hyperref[thm:morera_s_theorem]{Morera's Theorem}, we have that $f \in H(D)$ and since $D$ is arbitrary, $f \in H(\Omega)$ as required.

  Now since $R$ is arbitrary, we have that $f \in H(\mathbb{C})$, i.e. $f$ is entire.

  To find the zeros of $f$, let $z \in \Omega$. By \cref{thm:theorem_19} and in particular its remark, we have that
  \begin{equation*}
    \prod_{j = N_0}^{\infty} E_j \left( \frac{z}{a_j} \right) = 0 \iff E_n \left( \frac{z}{a_n} \right) = 0 \enspace \text{for some } n \geq N_0,
  \end{equation*}
  Therefore,
  \begin{equation*}
    \prod_{j=1}^{\infty} E_N \left( \frac{z}{a_j} \right) = 0 \iff E_n \left( \frac{z}{a_n} \right) = 0 \iff z = a_n
  \end{equation*}
  for some $n \in \mathbb{N}$. This shows that
  \begin{equation*}
    Z(f) = \{a_n\} \cap \Omega,
  \end{equation*}
  and since $R > 0$ is arbitrary, we have that $Z(f) = \{a_n : n \in \mathbb{N} \}$. This completes the proof. \qed
\end{proof}

\begin{eg}
  Construct an entire function with simple zeros at $z = 0, \pm 1, \pm 2, ...$, i.e. $z \in \mathbb{Z}$.

  \newthought{Solution 1}: Define $f_1(z) = \prod_{n=1}^{\infty} E_n \left(\frac{z}{n} \right)$, which is entire and has zeros at exactly $\mathbb{N}$ by \cref{thm:theorem_21}, and $f_2(z) = f_1(-z)$, which would have zeros at $- \mathbb{N}$. Then simply choose the function
  \begin{equation*}
    f(z) = z \cdot f_1(z) \cdot f_2(z)
  \end{equation*}
  so that we include the root $z = 0$.

  \newthought{Solution 2}: We can also choose
  \marginnote{\begin{ex}
      Check that $f$ indeed has its zeros at all integers.
  \end{ex}}
  \begin{equation*}
    f(z) = z \cdot \prod_{n=1}^{\infty} \left( 1 - \left( \frac{z}{n} \right)^2 \right)
  \end{equation*}
  Note that $\sin ( \pi z )$ is entire and has simple zeros at exactly $z \in \mathbb{Z}$. Then
  \begin{equation*}
    \frac{\sin (\pi z)}{f(z)} \text{ has no zeros or poles } \forall z,
  \end{equation*}
  and therefore by \cref{thm:theorem_18}, $\exists g \in H(\mathbb{C})$, such that
  \begin{equation*}
    \frac{\sin (\pi z)}{f(z)} = e^{g(z)}.
  \end{equation*}
  By the \hldefn{Product Formula of Sine}\sidenote{First proven by Euler, although incomplete.}\sidenote{It turns out $g(z) = \log \pi$.}, we have that
  \begin{equation*}
    \sin (\pi z) = \pi z \prod_{n=1}^{\infty} \left( 1 - \frac{z^2}{n^2} \right)
  \end{equation*}
  and hence
  \begin{equation*}
    f(z) = \prod_{n=1}^{\infty} \left( 1 - \frac{z^2}{n^2} \right) = \frac{\sin  (\pi z)}{\pi}.
  \end{equation*}
\end{eg}

% subsection weierstrass_products (end)

% section infinite_products_continued_2 (end)

% chapter lecture_32_apr_04_2018 (end)
