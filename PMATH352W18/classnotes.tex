...
\begin{thm}[Fundamental Theorem of Calculus]\label{thm:fundamental_theorem_of_calculus}
	Let $\gamma:[a,b] \to \mathbb{C}$ be a path inside an open set $\Omega \subseteq \mathbb{C}$. Suppose $f(z)$ is continuous on $\gamma$, and has an antiderivative $F$ which is holomorphic on $\Omega$ (i.e. $\forall z \in \Omega \; f(z) = F'(z)$ and $F$ is holomorphic in $\Omega$). Then
	\begin{equation}
