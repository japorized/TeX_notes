...
\begin{defn}[Mean Excess Loss]\index{Mean Excess Loss}
\label{defn:mean_excess_loss}
...
\begin{propo}[Relation of $\TVaR_p(X)$ and $e_X(d)$]
\label{propo:relation_of_tvar_p_x_and_e_x_d_}
  For a crv $X$, we have
  \begin{equation*}
...
\begin{propo}[Expection from Survival Function]
\label{propo:expection_from_survival_function}
...
\begin{propo}[Expected Deductible]
...
\begin{propo}[An Expression for Mean Excess Value]
...
\begin{defn}[Empirical Distribution Function]\index{Empirical Distribution Function}
\label{defn:empirical_distribution_function}
  Let $\{ X_1, ..., X_n \}$ be an iid sample of a risk $X$. Then its \hlnoteb{empricial distribution function (edf)} is defined as
  \begin{equation*}
...
\begin{figure}[h]
  \begin{tikzpicture}[
    block/.style = {rectangle, draw, rounded corners,
               ...
               color=base16-eighties-dark,fill=base16-eighties-blue},
    ...
    \node [sblock, left=of decision,color=base16-eighties-dark,fill=base16-eighties-red] (no) {\textbf{No}};
    \node [sblock, right=of decision,color=base16-eighties-dark,fill=base16-eighties-green] (yes) {\textbf{DONE!}};
  ...
  \end{tikzpicture}
  \caption{Process of a Parametric Approach}
  \label{fig:process_of_a_parametric_approach}
\end{figure}

\paragraph{Common Techniques in Creating New Parametric Distributions} Before diving into the topic, first, a definition:

\begin{defn}[Parametric Distribution]\index{Parametric Distribution}
\label{defn:parametric_distribution}
...
\begin{propo}[Multiplication by a Constant]
\label{propo:multiplication_by_a_constant}
...
\begin{defn}[Scale Distribution]\index{Scale Distribution}
\label{defn:scale_distribution}
...
\begin{crly}
\label{crly:constant_multiplication_crly}
...
\begin{defn}[Scale Parameter]\index{Scale Parameter}
\label{defn:scale_parameter}
...
\begin{propo}[Raising to a Power]
\label{propo:raising_to_a_power}
...
\begin{propo}[Exponentiation Method]
\label{propo:exponentiation_method}
...
\begin{defn}[Discrete Mixed Distribution]\index{Discrete Mixed Distribution}
\label{defn:discrete_mixed_distribution}
  Let $\Theta$ be a drv taking values on $\{ \theta_1, \theta_2, ..., \theta_n \}$ with
  \begin{equation*}
    P(\Theta = \theta_i) = p_i > 0, \quad i = 1, ..., n,
  \end{equation*}
...
\begin{defn}[Continuous Mixture]\index{Continuous Mixture}
\label{defn:continuous_mixture}
...
\begin{solution}
  ...
  \begin{align*}
    f_X(x) &= \int_{0}^{\infty} f_{X \mid \Lambda}(x \mid \lambda) f_{\Lambda}(\lambda) \dif{\lambda} \\
           ...
           &= \frac{\theta^\alpha \Gamma(\alpha + 1)}{\Gamma(\alpha) ( x + \theta )^{\apha + 1}} = \frac{\alpha \theta^\alpha}{(x + \theta)^{\alpha + 1}}.
  \end{align*}
\end{solution}

\begin{propo}[Total Expectation and Total Variance]
\index{Total Expectation}\index{Total Variance}
\label{propo:total_expectation_and_total_variance}
...
\end{equation*}
Now if $f_X$ takes on large values for large $x$, we may have $E\left[X^k\right]$ blow up to infinity, and so it is desirable to find/use some distribution with a \hlnotea{decaying} probability function.
