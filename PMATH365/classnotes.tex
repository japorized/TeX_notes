...
\begin{defn}[Differential]\index{Differential}\label{defn:differential}
  ...
  is a linear map $(\dif{f})_{x_0} : \mathbb{R}^n \to \mathbb{R}^m$, or an
  $m \times n$ real matrix, given by
  \begin{equation*}
    (\dif{f})_{x_0} = \begin{pmatrix}
  ...
  \textsuperscript{th} entry of $(\dif{f})_{x_0}$ is
  $\frac{\partial f^i}{\partial x^j}(x_0)$. $(\dif{f})_{x_0}$ is also called
...
\begin{propo}[Differential of the Identity Map is the Identity Matrix]\label{propo:differential_of_the_identity_map_is_the_identity_matrix}
  ...
  $f(x) = x$. Then $(\dif{f})_{x_0} = I_n$, the $n \times n$ matrix, then for
  any $x_0 \in U$.
\end{propo}
  ...
  \begin{equation*}
    \underset{m \times 1}{f(x)}
      = \underset{m \times 1}{f(x_0)}
        + \underset{m \times n}{(\dif{f})_{x_0}} \underset{n \times 1}{(x - x_0)}
...
\begin{note}
  ...
  $f'(x_0) = (\dif{f})_{x_0} = \frac{\dif{f}}{\dif{x}}(x_0)$.
\end{note}
...
\begin{thm}[The Chain Rule]\index{The Chain Rule}\label{thm:the_chain_rule}
  ...
  \begin{equation}\label{eq:the_chain_rule}
    (\dif{(g \circ f)})_{x_0} = (\dif{g})_{f(x_0)} (\dif{f})_{x_0}.
  \end{equation}
\end{thm}
...
\begin{eg}
  ...
  \begin{equation*}
    ...
    = (\partial_i f)(p) = \frac{\partial f}{\partial x^i}(p) = (f \circ \gamma_{v_p})'(p)
  \end{equation*}
...
\begin{proof}
  By the chain rule,
  \begin{equation*}
    (f \circ \phi)'(0) = (\dif{(f \circ \phi)})_0 = (\dif{f})_{\phi(0)}(\dif{\phi})_0 
      = (\dif{f})_{\phi(0)} \phi'(0),
  \end{equation*}
