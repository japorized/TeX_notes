...
\begin{defn}[Maximal Rank]\index{Maximal Rank}\label{defn:maximal_rank}
  ...
  say that $F$ has \hlnoteb{maximal rank} on $U$ if the
...
\begin{proof}
  ...
  $\rho_i(U_i)$ \sidenote{Recall that $\rho_i$ is defined such that
  \begin{equation*}
    \rho_i(p) = \frac{\zeta_i(p)}{\sum_{i=1}^{m} \zeta_i(p)}
  \end{equation*}
  where
  \begin{equation*}
...
\begin{proof}[Stokes' Theorem (Continued)]
  ...
  \sidenote{\hlwarn{What in the world allowed us to make such a claim?}}, and
  ...
  \noindent
  \hlbnoted{Case 1} Suppose $U$ is open in $\mathbb{R}^k$, with $U \cap \partial
  ...
  \noindent
  \hlbnoted{Case 2} Suppose $U$ is not open in $\mathbb{R}^k$. Then $U \cap
...
\begin{defn}[Inner Product]\index{Inner Product}\label{defn:inner_product}
  Let $V$ be $n$-dimensional real vector space. An \hlnoteb{inner product} on
  ...
  such that
  \begin{enumerate}
    \item (\hlnotea{bilinearity}) $\langle v, w \rangle$ is linear in $v$ and linear in $w$;
    \item (\hlnotea{symmetry}) $\langle v, w \rangle = \langle w, v \rangle$ ; and
    \item (\hlnotea{positive definite}) $\langle v, v \rangle \geq 0$ and $\langle v, v
      \rangle = 0 \iff v = 0$.
  \end{enumerate}
\end{defn}
...
\begin{defn}[Hodge Star Operator]\index{Hodge Star Operator}\label{defn:hodge_star_operator}
  ...
  space. Let $\alpha \in \Lambda^k(V)$, where $k \leq n$. The \hlnoteb{Hodge
  Star Operator} $*$ is a map
  \begin{equation*}
...
Let $\hat{I}$ be the \hlnotea{complementary} \sidenote{For example, if $I = (1,
3, 5)$ in $n = 5$, then $\hat{I} = (2, 4)$.} multi-index of $I$.
...
\begin{defn}[Isometry]\index{Isometry}\label{defn:isometry}
  A map $\phi : X \to Y$ is called an \hlnoteb{isometry} if
  \begin{equation*}
...
\begin{crly}[Alternative Definition of the Hodge Star Operator]\label{crly:alternative_definition_of_the_hodge_star_operator}
...
% section physical_and_geometric_interpretations_of_stokes_theorem
...
% subsection inner_product_on_the_tangent_space_of_a_submanifold
...
\hldefn{normal space} at $p$ of $M$. $N_p M$ is a $(n - k)$-dimensional vector
space.

% subsection inner_product_on_the_tangent_space_of_a_submanifold (end)

% section physical_and_geometric_interpretations_of_stokes_theorem (end)
...
% section physical_and_geometric_interpretations_of_stokes_theorem_continued

\subsection{Volume Form}%
\label{sub:volume_form}
% subsection volume_form

\begin{defn}[Volume Form]\index{Volume Form}\label{defn:volume_form}
  Suppose $M$ is oriented. Then we can choose an \hlnotea{oriented orthonormal
  basis} $\{ e_1, \ldots, e_k \}$ of $T_p M$. Then the
  \hyperref[defn:dual_map]{dual basis} is also oriented and orthonormal.
  
  This gives us a \hlimpo{preferred} orientation form $\mu \in \Omega^k(M)$ such
  that at any $p \in M$, we have
  \begin{equation*}
    \mu_p = e^1 \land \hdots \land e^k,
  \end{equation*}
  which we call the \hlnoteb{volume form}. \sidenote{A volume form is sort of
  like a higher-dimensional way of naming a "volume".}
\end{defn}
...
\begin{defn}[Metric dual 1-form]\index{Metric dual 1-form}\label{defn:metric_dual_1_form}
  Given a smooth vector field $X$ on $M$, we get a \hlnoteb{``metric dual''
  1-form} $X^\sharp$ on $M$ defined by
  \begin{equation*}
...
\begin{defn}[Metric dual vector field]\index{Metric dual vector field}\label{defn:metric_dual_vector_field}
  Given a smooth $1$-form $\alpha$ on $M$, we get a \hlnoteb{``metric dual'' vector
  field} $\alpha^\flat$ on $M$ defined by
  \begin{equation*}
...
\begin{defn}[Local Frame]\index{Local Frame}\label{defn:local_frame}
  ...
  is a $k$-dimensional submanifold, we call $\mathcal{A}$ a \hlnoteb{local
  frame} on $M$ if
  \begin{equation*}
...
% subsection musical_isomorphisms (end)

% section physical_and_geometric_interpretations_of_stokes_theorem_continued (end)

% chapter lecture_33_apr_03rd (end)
