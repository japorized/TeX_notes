...
\begin{proof}
  ...
  \alpha_j$ is smooth.\qed\
\end{proof}

\begin{remark}
  Again, we see that this characterization is independent of the chooice of basis.
\end{remark}
...
\begin{note}
  ...
  \end{equation*}
  where $X^j = \delta^{ij}$. Then if $\alpha = \alpha_k \hat{e}^k$ is a $1$-form, we have that
...
\chapter{Lecture 13 Feb 4th}%
\label{chp:lecture_13_feb_4th}
% chapter lecture_13_feb_4th
...
\Lambda^k(T_p^* \mathbb{R}^n)$ as in \cref{eq:normal_pullback_as_guide}, where we let $T = T_p
...
\begin{defn}[Pullback by $F$ of a $k$-Form]\index{Pullback}\label{defn:pullback_by_f_of_a_k_form}
  ...
  \begin{equation*}
    ...
    = \eta_{F(p)} ((\dif{F})_p(X_1)_p, \ldots, (\dif{F})_p(X_k)p).
  \end{equation*}
\end{defn}
...
\begin{proof}
  ...
  we have that $(F^* \eta)_{j_1, \ldots, j_k}$ is smooth, as required.\qed\
\end{proof}
...
% chapter lecture_13_feb_4th (end)
