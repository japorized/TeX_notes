...
\begin{defn}[Parametrizations and Parametrized Submanifolds]\index{Parametrization}\index{Parametrized Submanifold}\label{defn:parametrizations_and_parametrized_submanifolds}
  An immersion $\phi : U \subseteq \mathbb{R}^k \to \mathbb{R}^n$ that is also a
  homeomorphism onto its image is called a \hlnoteb{parametrization}. The image
...
\begin{lemma}[Parametrized Submanifolds are not Determined by Immersions]\label{lemma:parametrized_submanifolds_are_not_determined_by_immersions}
...
\begin{defn}[$j$\textsuperscript{th} Coordinate Curve]\index{$j$\textsuperscript{th} Coordinate Curve}\label{defn:_j_th_coordinate_curve}
  ...
  \hlnoteb{$j$\textsuperscript{th} coordinate curve} of the parametrization
...
\begin{note}
  ...
  parametrized submanifold} of $\mathbb{R}^n$ \sidenote{Some authors call a
  $k$-dimensional submanifold a \hlnotea{regular submanifold} of $\mathbb{R}^n$,
  and use the term \hlnotea{regular map} for a parametrization.}.
\end{note}
...
% section submanifolds_in_terms_of_local_parametrizations_continued
...
\begin{defn}[Local Parametrizations]\index{Local Parametrizations}\label{defn:local_parametrizations}
  ...
  \hlnoteb{local parametrization} of $M$, and the collection
  \begin{equation*}
    \{ \phi_\alpha : U_\alpha \to V_\alpha \cap M : \alpha \in A \}
  \end{equation*}
  of local parametrizations is called a \hldefn{cover} of $M$. Given any such
  ...
  \hldefn{allowable local parametrization}. The set of all possible allowable
  local parametrizations under a given cover is called the \hldefn{maximal
  cover} of the cover.
\end{defn}
...
% section submanifolds_in_terms_of_local_parametrizations_continued (end)
...
\marginnote{
\begin{remark}
  Notice that \cref{eg:parametrization_of_s_n_1_in_r_n_no2} is a much faster way
  than \cref{eg:parametrization_of_s_n_1_in_r_n_no1} to finding a cover!
\end{remark}}
...
\begin{thm}[Points on the Parametrization]\index{Points on the Parametrization}\label{thm:points_on_the_parametrization}
...
\begin{note}
  ...
  $(\dif{\phi})_u$ of a parametrization is \hlbnotec{independent of any
  parametrization} such that $\phi(u) = p$.
\end{note}
...
\begin{note}
  ...
  is the same regardless of the choice of parametrization.
\end{note}
...
\begin{lemma}[Correspondence of Smooth Maps between a Submanifold and Its Parametrization]\label{lemma:correspondence_of_smooth_maps_between_a_submanifold_and_its_parametrization}
...
\begin{lemma}[Composition of Pullbacks of Transition Maps and Parametrizations]\label{lemma:composition_of_pullbacks_of_transition_maps_and_parametrizations}
...
\marginnote{\cref{crly:_r_forms_on_a_submanifold_and_its_parametrizations_are_equivalent}
tells us that the $r$-forms on $M$ stays consistent across the different
parametrizations, and this equivalence comes from the transition map between
parametrizations.}
\begin{crly}[$r$-forms on a Submanifold and Its Parametrizations are Equivalent]\label{crly:_r_forms_on_a_submanifold_and_its_parametrizations_are_equivalent}
...
\begin{defn}[Compatible Orientation]\index{Compatible Orientation}\label{defn:compatible_orientation}
  ...
  parametrization for $M$ with image $V \cap M$. Let $\nu$ be the orientation on
  ...
  C^\infty(V \cap M)$. We say that the local parametrization $\phi$ is
...
\hlnotea{compactly supported} in the image of the single parametrization.
...
\begin{proof}[Stokes' Theorem (Continued)]
  ...
  \restriction_{\hat{A}} \hat{A} \to \mathbb{R}^n$ be a parametrizations. Let
...
canonical isomorphism. % TODO : reference required, you know what this is
