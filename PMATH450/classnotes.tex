...
\begin{propo}[Kernel of a Vector Space is a Linear Manifold]\label{propo:kernel_of_a_vector_space_is_a_linear_manifold}
  ...
  Hilbert space, which is a vector space endowed with an inner product such that
  with an appropriate norm, turns it into a complete metric space, is called a
  linear manifold if it is closed under addition and scalar multiplication.
...
\begin{defn}[$\mathcal{L}_\infty(E, \mathbb{K})$]\index{$\mathcal{L}_\infty(E, \mathbb{K})$}\label{defn:_l_infty_e_k_}
...
\begin{eg}
  \begin{enumerate}
    ...
    \abs{f(x)}}$} We know that every continuous function on a measurable set is
    measurable \sidenote{cf.
    \cref{propo:continuous_functions_on_a_measurable_set_is_measurable}}, so $f
...
\begin{proof}
  ...
  \kappa \in \mathbb{K}$. It is clear that $\kappa f \in \mathcal{L}(E,
  \mathbb{K})$, and we quickly notice that
  \begin{align*}
    \nu_\infty(\kappa f)
  ...
  \mathbb{K}$. As noted before, if $\kappa = 0$, then $\kappa f = 0 \in
  \mathcal{L}_\infty(E, \mathbb{K})$.

  Now suppose $f, g \in \mathcal{L}_\infty(E, \mathbb{K})$. WTS
  \begin{equation*}
    \nu_\infty(f + g) \leq \nu_\infty(f) + \nu_\infty(g).
  \end{equation*}
  Let $\alpha > \nu_\infty(f)$ and $\beta > \nu_\infty(g)$. Let
  ...
  \end{equation*}
  Then $mE_f = 0 = mE_g$. Let $H = E \setminus (E_f \cup E_g)$. For $x \in H$,
  we have
  \begin{equation*}
  ...
  \begin{equation*}
    ...
    mE_g = 0,
  \end{equation*}
  and so $\nu_\infty(f + g) \leq \alpha + \beta$. Since $\alpha$ and $\beta$ 
...
\begin{thm}[$L_\infty(E, \mathbb{K})$ is a normed-linear space]\label{thm:_l_infty_e_k_is_a_normed_linear_space}
  Let $E \in \mathfrak{M}(\mathbb{R})$. Then $L_\infty(E, \mathbb{K})$ is a
  normed-linear space, where for $[f] \in L_\infty(E, \mathbb{K})$ we set
  \begin{equation*}
