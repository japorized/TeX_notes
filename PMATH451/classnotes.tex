...
\begin{homework}[Homework 13]\label{homework:13}
  ...
  is known as the \hldefn{symmetric difference} of $E$ and $A$.)
\end{homework}
...
% chapter lecture_17_oct_11th_2019

\section{Modes of Convergences (Continued)}%
\label{sec:modes_of_convergences_continued}
% section modes_of_convergences_continued

\begin{defn}[Cauchy in Measure]\index{Cauchy in measure}\label{defn:cauchy_in_measure}
  Let $(X, \mathfrak{M}, \mu)$ be a measure space, and
  $f_n, f : X \to \mathbb{R}_e$ be measurable functions.
  We say that $\{ f_n \}$ is \hlnoteb{Cauchy in measure}
...
\begin{propo}[$\mathcal{L}^1$-convergence implies Convergence in Measure]\label{propo:_mathcal_l_1_convergence_implies_convergence_in_measure}
  Let $(X ,\mathfrak{M}, \mu)$ be a measure space, and
...
\begin{thm}[Various results related to convergence in measure]\label{thm:various_results_related_to_convergence_in_measure}
  Let $(X, \mathfrak{M}, \mu)$ be a measure space,
...
\begin{proof}
  \begin{enumerate}
    ...
    \item \hlwarn{Something's wrong with the prove given in class. Will confirm.}
  \end{enumerate}
\end{proof}
...
\begin{homework}[Homework 14]\label{homework:14}
  ...
  \end{equation*}
  \sidenote{We may say that each $f_n$ is bounded by $M$ a.e.}
  Prove that
  \begin{equation*}
...
% section modes_of_convergences_continued (end)

% chapter lecture_17_oct_11th_2019 (end)

\chapter{Lecture 18 Oct 23rd 2019}%
\label{chp:lecture_18_oct_23rd_2019}
% chapter lecture_18_oct_23rd_2019

\section{Modes of Convergences (Continued 2)}%
\label{sec:modes_of_convergences_continued_2}
% section modes_of_convergences_continued_2

\begin{thm}[Egoroff's Theorem]\index{Egoroff's Theorem}\label{thm:egoroff_s_theorem}
...
% section modes_of_convergences_continued_2 (end)

\section{Product Measures}%
\label{sec:product_measures}
% section product_measures

\begin{note}[Motivation for looking at Product Measures of $\sigma$-finite Measures]
...
\begin{note}[A sketch of how we can always construct $\lambda$]
  ...
  \end{ex}
  By \nameref{thm:caratheodory_s_theorem}, the set
  \begin{equation*}
...
\begin{eg}[How $(2)$ fails when one of the measures is not $\sigma$-finite]
  Consider $X = Y = [0, 1]$,
...
\begin{propo}[Component-wise Measurability of Functions and Sets]\label{propo:component_wise_measurability_of_functions_and_sets}
...
\begin{defn}[Monotone Classes]\index{Monotone Classes}\label{defn:monotone_classes}
  Let $X$ be a non-empty set.
  $\xi \subseteq \mathcal{P}(X)$ is called a \hlnoteb{monotone class}
  if
  \begin{enumerate}
...
\begin{thm}[Monotone Class Theorem]\index{Monotone Class Theorem}\label{thm:monotone_class_theorem}
...
\begin{lemma}[Lemma for Monotone Class Theorem]\label{lemma:lemma_for_monotone_class_theorem}
...
% section product_measures (end)

% chapter lecture_18_oct_23rd_2019 (end)

\chapter{Lecture 19 Oct 25th 2019}%
\label{chp:lecture_19_oct_25th_2019}
% chapter lecture_19_oct_25th_2019

\section{Product Measures (Continued)}%
\label{sec:product_measures_continued}
% section product_measures_continued

\begin{proof}[Proof for \nameref{thm:monotone_class_theorem}]
  ...
  \sidenote{The proof gets really slippery from hereon.}
  Let $E \in \zeta$. Set
  \begin{equation*}
  ...
  \noindent
  \hlbnotec{Claim:  $\zeta_E$ is a monotone class}
  ...
  \noindent
  \hlbnotec{$\zeta_A = \zeta$ for any $A \in \mathcal{A}$}
  ...
  \noindent
  \hlbnotec{$\zeta_E = \zeta$ for any $E \in \zeta$}
...
\begin{thm}[Existence of Product Measures for $\sigma$-finite Measure Spaces]\label{thm:existence_of_product_measures_for_sigma_finite_measure_spaces}
...
\marginnote{
\begin{mnote}
  It is often useful to think of $A \times B$ as a rectangle.
\end{mnote}
}
...
\begin{proof}
  ...
  Let
  \begin{equation*}
    \zeta \coloneqq \{ E \in \mathfrak{M} \otimes \mathfrak{N}
        : \text{ (1) and (2) are true } \}.
  \end{equation*}
  Let $\mathcal{A}$ be the algebra generated by
  $\{ A \times B : A \in \mathfrak{M}, B \in \mathfrak{N} \}$.

  \noindent
  \sidenote{We want to show that we may write each element of $\mathcal{A}$
  as a disjoint union of rectangles, each of which has components
  in $\mathfrak{M}$ and $\mathfrak{N}$.}
  \hlbnotec{$F \in \mathcal{A} \implies F = \bigcupdot_{i=1}^{n} (A_i \times
  B_i),$ where $ A_i \in \mathfrak{M},\, B_i \in \mathfrak{N}$}
  Note that for any $F \in \mathcal{A}$, we may write
  $F = A \times B$ for some $A \in \mathfrak{M}$ and $B \in \mathfrak{N}$.
  As stated, note that $\mathcal{A}$ is an algebra.

  If $F = (A \times B)^C$, then we may write
  \begin{equation*}
    (A \times B)^C = (A^C \times Y) \cupdot (A \times B^C)
  \end{equation*}
  (see \cref{fig:idea_of_partitioning_a_times_b_c_}).
  \begin{marginfigure}
    \centering
    \begin{tikzpicture}
      ...
      \draw[color=green,dashed] (-1, 2) -- (-1, -2);
      \draw[color=green,dashed] (1, 2) -- (1, -2);
      \fill[color=green!30!background] (-2, 2) rectangle (-1, -2);
      \fill[color=green!30!background] (1, 2) rectangle (2, -2);
      \node[color=green] at (0, -2.5) {\mbox{\boldmath$A^C \times Y$}};

      \draw[color=orange,dashed] (-1, 2) -- (-1, 0.5) -- (1, 0.5) -- (1, 2);
      \draw[color=orange,dashed] (-1, -2) -- (-1, -0.5) -- (1, -0.5) -- (1, -2);
      \fill[color=orange!30!background] (-1, 2) rectangle (1, 0.5);
      \fill[color=orange!30!background] (-1, -2) rectangle (1, -0.5);
      \node[color=orange] at (0, -1.5) {\mbox{\boldmath$A \times B^C$}};
    \end{tikzpicture}
    \caption{Idea of partitioning $(A \times B)^C$.}\label{fig:idea_of_partitioning_a_times_b_c_}
  \end{marginfigure}
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \fill[color=magenta,fill opacity=0.3] (0, 1.5) rectangle (1, -1);
      \node[color=magenta] at (0.5, -1.5)
        {\mbox{\boldmath$(A_1 \cap A_2) \times (B_1 \cap B_2)$}};
      \fill[color=green,fill opacity=0.3] (-1, 1) rectangle (0, -1);
      \node[color=green] at (-1, 1.25)
        {\mbox{\boldmath$(A_1 \setminus A_2) \times B_1$}};
      \fill[color=orange,fill opacity=0.3] (1, 1.5) rectangle (2, -0.5);
      \node[color=orange] at (1.5, 1.75)
        {\mbox{\boldmath$(A_2 \setminus A_1) \times B_2$}};
    \end{tikzpicture}
  ...
  \hlwarn{Require clarification before proceeding, cause I have no idea
  why we even did anything.}
\end{proof}

% section product_measures_continued (end)

% chapter lecture_19_oct_25th_2019 (end)

\chapter{Lecture 20 Oct 28th 2019}%
\label{chp:lecture_20_oct_28th_2019}
% chapter lecture_20_oct_28th_2019

\section{Product Measures (Continued 2)}%
\label{sec:product_measures_continued_2}
% section product_measures_continued_2

\begin{thm}[Fubini-Tonelli Theorem]\index{Fubini-Tonelli Theorem}\label{thm:fubini_tonelli_theorem}
  Let $(X, \mathfrak{M}, \mu)$ and $(Y, \mathfrak{N}, \nu)$
  be $\sigma$-finite measure spaces.
  \begin{enumerate}
...
\begin{eg}[Failure of Fubini's Theorem when $f \notin \mathcal{L}^1(X \times Y)$]\label{eg:fubini_failure_not_mu_times_nu_integrable}
  Consider the function
  \begin{equation*}
    f(x, y) = \begin{cases}
      1 & x \leq y \leq x + 1 \\
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \draw (0, 0) -- (2, 2);
      \draw (1, 0) -- (2, 1);
      \fill[color=green,fill opacity=0.3]
        (0, 1) -- (1, 2) -- (2, 2) -- (0, 0);
      \node at (1.2, 1.7) {1};
      \fill[color=orange,fill opacity=0.3]
        (0, 0) -- (2, 2) -- (2, 1) -- (1, 0);
      \node at (1.2, 0.7) {-1};

      \draw[line width=1.5pt,dashed,color=red] (1, 2.2) -- (1, -0.2)
        node[below,color=foreground] {1};
      \draw[line width=1.5pt,dashed,color=blue] (2.2, 1) -- (-0.2, 1)
        node[left,color=foreground] {1};
    \end{tikzpicture}
...
\begin{proof}[Tonelli]
  \hlbnoted{Case 1: Characteristic Functions}
...
\begin{thm}[Fubini-Tonelli Theorem for Complete Measures]\index{Fubini-Tonelli Theorem for Complete Measures}\label{thm:fubini_tonelli_theorem_for_complete_measures}
  Let $(X, \mathfrak{M}, \mu)$ and $(Y, \mathfrak{N}, \nu)$
...
% section product_measures_continued_3 (end)

\section{Signed Measures}%
\label{sec:signed_measures}
% section signed_measures
...
\begin{defn}[Signed Measure]\index{Signed Measure}\label{defn:signed_measure}
  Let $(X, \mathfrak{M})$ be a measurable space.
  We say that $\nu : X \to \mathbb{R}_e$ is a \hlnoteb{signed measure}
  if
  \begin{enumerate}
    \item $\nu(\emptyset) = 0$;
    \item $\nu$ can take on all values in $\mathbb{R}_e$,
      but \hlnotec{not} both $\infty$ and $-\infty$; and
...
\begin{defn}[$\nu$-positive and $\nu$-negative]\index{$\nu$-positive}\index{$\nu$-negative}\label{defn:_nu_positive_and_nu_negative}
  Let $(X, \mathfrak{M})$ be a measurable space,
  and $\nu$ a signed measure.
  We say that $E \in \mathfrak{M}$ is \hlnoteb{$\nu$-positive}
...
% section signed_measures (end)

% chapter lecture_21_oct_30th_2019 (end)

\chapter{Lecture 22 Nov 1st 2019}%
\label{chp:lecture_22_nov_1st_2019}
% chapter lecture_22_nov_1st_2019

\section{Hahn Decomposition}%
\label{sec:hahn_decomposition}
% section hahn_decomposition

\begin{thm}[Hahn Decomposition Theorem]\index{Hahn Decomposition Theorem}\label{thm:hahn_decomposition_theorem}
...
\marginnote{
\begin{mnote}
  Recall that $P \triangle P'$ is called the
  \hyperref[homework:13]{symmetric difference} of $P$ and $P'$,
  and it is defined as
  \begin{equation*}
    P \triangle P' = (P \setminus P') \cup (P' \setminus P).
  \end{equation*}
\end{mnote}
}
...
\begin{lemma}[Lemma for Hahn Decomposition]\label{lemma:lemma_for_hahn_decomposition}
...
\begin{proof}[Proof for \nameref{thm:hahn_decomposition_theorem}]
...
% section hahn_decomposition (end)

% chapter lecture_22_nov_1st_2019 (end)

\chapter{Lecture 23 Nov 4th 2019}%
\label{chp:lecture_23_nov_4th_2019}
% chapter lecture_23_nov_4th_2019

\section{Hahn Decomposition (Continued)}%
\label{sec:hahn_decomposition_continued}
% section hahn_decomposition_continued

\begin{defn}[Hahn Decomposition]\index{Hahn Decomposition}\label{defn:hahn_decomposition}
  Let $\nu$ be a signed measure, and $X = P \cupdot N$,
  where $P$ is $\nu$-positive and $N$ is $\nu$-negative.
  We call $P \cupdot N$ the \hlnoteb{Hahn decomposition} of $\nu$.
\end{defn}
...
\begin{defn}[Mutually Singular]\index{Mutually Singular}\label{defn:mutually_singular}
  Let $(X, \mathfrak{M})$ be a measurable space,
  with signed measures $\mu$ and $\nu$.
  The measures $\mu$ and $\nu$ are said to be \hlnoteb{mutually singular},
...
\begin{thm}[Jordan Decomposition Theorem]\index{Jordan Decomposition Theorem}\label{thm:jordan_decomposition_theorem}
...
\begin{proof}
  ...
  By the \nameref{thm:hahn_decomposition_theorem}, we have that
  \begin{equation*}
...
\begin{defn}[Positive and Negative Variation of a Signed Measure]\index{Positive variation of a signed measure}\index{Negative variation of a signed measure}\label{defn:positive_and_negative_variation_of_a_signed_measure}
  Let $(X, \mathfrak{M})$ be a measurable space and $\nu$ a signed measure
  of the space. By the \nameref{thm:jordan_decomposition_theorem},
  we may write $\nu = \nu^+ - \nu^-$ such that $\nu^+ \bot \nu^-$.
  We call $\nu^+$ the \hlnoteb{positive variation of $\nu$}
  and $\nu^-$ the \hlnoteb{negative variation of $\nu$}.
\end{defn}

\begin{defn}[Total Variation of a Signed Measure]\index{Total variation of a signed measure}\label{defn:total_variation_of_a_signed_measure}
  By the same assumption as the above, we call
  \begin{equation*}
    \abs{\nu} = \nu^+ + \nu^-
  \end{equation*}
  the \hlnoteb{total variation of $\nu$}.
\end{defn}
...
% section hahn_decomposition_continued (end)
...
% section radon_nikodym_theorem_and_the_Lebesgue_decomposition_theorem

\begin{defn}[Absolutely Continuous Measure]\index{Absolutely Continuous Measure}\label{defn:absolutely_continuous_measure}
  Let $(X, \mathfrak{M})$ be a measurable space.
  Let $\nu$ be a signed measure and $\mu$ a positive measure.
  We say that $\nu$ is \hlnoteb{absolutely continuous with respect to} $\mu$,
...
\begin{lemma}[Equivalent Definitions of a Absolutely Continuous Measure]\label{lemma:equivalent_definitions_of_a_absolutely_continuous_measure}
  Let $(X, \mathfrak{M})$ be a measurable space,
...
\begin{thm}[Alternative Definition for Absolute Continuity of Measures]\label{thm:alternative_definition_for_absolute_continuity_of_measures}
...
% section radon_nikodym_theorem_and_the_lebesgue_decomposition_theorem (end)

% chapter lecture_23_nov_4th_2019 (end)

\chapter{Lecture 24 Nov 6th 2019}%
\label{chp:lecture_24_nov_6th_2019}
% chapter lecture_24_nov_6th_2019
...
% section radon_nikodym_theorem_and_the_lebesgue_decomposition_theorem_continued

\begin{crly}[On a special signed measure]\label{crly:on_a_special_signed_measure}
...
\begin{lemma}[Relationship Between Two Finite Positive Measure]\label{lemma:relationship_between_two_finite_positive_measure}
  Let $\mu$ and $\nu$ be finite positive measures
  on the measurable space $(X, \mathfrak{M})$.
  Then either $\mu \bot \nu$, or \sidenote{This is an exclusive or statement.}
...
\begin{thm}[The Lebesgue Decomposition Theorem and The Radon-Nikodym Theorem]\index{Lebesgue Decomposition Theorem}\index{Radon-Nikodym Theorem}\label{thm:lebesgue_decomposition_theorem}\label{thm:radon_nikodym_theorem}
  Let $(X, \mathfrak{M})$ be a measurable space,
  ...
  (\hlnoteb{Lebesgue Decomposition}).
  Moreover, $\exists f : X \to [0, \infty]$ that is $\mathfrak{M}$-measurable such that
  \begin{equation*}
    \rho(E) = \int_{E} f \dif{\mu}
  \end{equation*}
  (\hlnoteb{Radon-Nikodym}).
\end{thm}

\marginnote{This is a constructive proof. It tells us exactly
how to create $\lambda$ and $\rho$.}

\begin{proof}
  \hlbnoted{Case 1: $\mu$ and $\nu$ are both finite}
  Let
  \begin{equation*}
  ...
  \noindent
  \hlbnotec{$\mathcal{F}$ is closed under $\max$ function}
  ...
  \noindent
  \hlbnotec{$\mathcal{F}$ contains limits of its sequences}
  Let
  \begin{equation*}
  ...
  \noindent
  \hlbnoted{Case 2: $\nu$ and $\mu$ are both $\sigma$-finite}
  \sidenote{We shall strict the finite pieces together and then try
  paying attention to some of the technical details.}
...
% section radon_nikodym_theorem_and_the_lebesgue_decomposition_theorem_continued (end)

% chapter lecture_24_nov_6th_2019 (end)

\chapter{Lecture 25 Nov 8th 2019}%
\label{chp:lecture_25_nov_8th_2019}
% chapter lecture_25_nov_8th_2019
...
% section radon_nikodym_theorem_and_the_lebesgue_decomposition_theorem_continued_2

\begin{crly}[A Corollary to Radon-Nikodym]\label{crly:a_corollary_to_radon_nikodym}
  ...
  \begin{enumerate}
    \item Let $F : \mathbb{R} \to \mathbb{R}$ such that
      ...
      \begin{equation*}
        ...
        \end{cases}.
      \end{equation*}
      Notice that $f$ is the \hlnoteb{derivative} of $F_1$.
      By the
      \nameref{thm:agreement_of_riemann_integration_and_lebesgue_integration_for_bounded_functions},
      we know
      \begin{equation*}
      ...
      given in \nameref{thm:lebesgue_decomposition_theorem},
    ...
    \item Recall the \nameref{para:cantor_function}, which here we shall call it
      $F_C$.
      Consider the function $F : \mathbb{R} \to \mathbb{R}$ such that
      \begin{equation*}
        F(x) = \begin{cases}
...
\begin{defn}[Derivative]\index{Derivative}\label{defn:derivative}
...
\begin{note}[Some formulae]
  \begin{enumerate}
    ...
    \item (\hlnoteb{Change of variables})
      Suppose $\nu \ll \mu \ll \nu$.
      ...
      \begin{equation*}
        ...
        g = \frac{\dif{\mu}}{\dif{\nu}}.
      \end{equation*}
      By the Midterm problem, \sidenote{This is the worst measure I've seen.}
      \begin{equation*}
...
% section radon_nikodym_theorem_and_the_lebesgue_decomposition_theorem_continued_2 (end)

% chapter lecture_25_nov_8th_2019 (end)

\chapter{Lecture 26 Nov 11th 2019}%
\label{chp:lecture_26_nov_11th_2019}
% chapter lecture_26_nov_11th_2019

\section{Non-decreasing Functions}%
\label{sec:non_decreasing_functions}
% section non_decreasing_functions
...
\begin{propo}[Properties of $F(x+)$ and $F(x-)$]\label{propo:properties_of_f_x_plus_and_f_x_minus}
  Let $F : \mathbb{R} \to \mathbb{R}$ be an increasing function.
  \begin{enumerate}
...
% section non_decreasing_functions (end)

\section{Vitali Covering Lemma}%
\label{sec:vitali_covering_lemma}
% section vitali_covering_lemma
...
\begin{defn}[Vitali Covering]\index{Vitali Covering}\label{defn:vitali_covering}
  Let $E \subseteq \mathbb{R}$. We say that $\mathfrak{I}$
  is a \hlnoteb{Vitali covering} of $E$ if $\forall \epsilon > 0$,
...
\begin{remark}
  ...
  \href{https://tex.japorized.ink/PMATH351F18/classnotes.pdf\#defn.70}{\hlnoteb{cover}} for $E$,
  and also a special kind of
  \href{https://tex.japorized.ink/PMATH351F18/classnotes.pdf\#defn.75}{\hlnoteb{$\epsilon$-net}}.
\end{remark}

\begin{thm}[Vitali Covering Lemma]\index{Vitali Covering Lemma}\label{thm:vitali_covering_lemma}
...
\begin{proof}
  ...
  % Since $m^*(E) < \infty$, 
  \hlwarn{Will come back}
\end{proof}

% section vitali_covering_lemma (end)

% chapter lecture_26_nov_11th_2019 (end)

\chapter{Lecture 27 Nov 13th 2019}%
\label{chp:lecture_27_nov_13th_2019}
% chapter lecture_27_nov_13th_2019

\section{Vitali Covering Lemma (Continued)}%
\label{sec:vitali_covering_lemma_continued}
% section vitali_covering_lemma_continued

\begin{defn}[$D^+, D^-, D_+, D_-$]\index{$D^+$}\index{$D^-$}\index{$D_+$}\index{$D_-$}\label{defn:d_upper_plus}\label{defn:d_upper_minus}\label{defn:d_lower_plus}\label{defn:d_lower_minus}
...
\begin{thm}[Partial Fundamental Theorem of Calculus for Increasing Functions]\label{thm:partial_fundamental_theorem_of_calculus_for_increasing_functions}
...
% section vitali_covering_lemma_continued (end)

\section{Functions of Bounded Variation}%
\label{sec:functions_of_bounded_variation}
% section functions_of_bounded_variation

\begin{defn}[Functions of Bounded Variation]\index{Functions of Bounded Variation}\label{defn:functions_of_bounded_variation}
  Let $F : [a, b] \to \mathbb{C}$. \sidenote{Note that we use $\mathbb{C}$ here,
  so that we allow for the function to map to both complex and also purely real
  values.}
  We say that $F$ is of \hlnoteb{bounded variation} if
  \begin{equation*}
  ...
  We call $V_F[a, b]$ the \hldefn{variation} of $F$ on $[a, b]$.
...
% section functions_of_bounded_variation (end)

% chapter lecture_27_nov_13th_2019 (end)

\chapter{Lecture 28 Nov 15th 2019}%
\label{chp:lecture_28_nov_15th_2019}
% chapter lecture_28_nov_15th_2019

\section{Functions of Bounded Variation (Continued)}%
\label{sec:functions_of_bounded_variation_continued}
% section functions_of_bounded_variation_continued

\begin{defn}[Total Variation]\index{Total Variation}\label{defn:total_variation}
  Let $F$ be a function of bounded variation.
  The \hlnoteb{total variation} of $F$ is defined as
  \begin{equation*}
...
\begin{lemma}[Total Variation $\pm F$]\label{lemma:total_variation_pm_f_}
...
\begin{note}[Jordan Decomposition of $F$]\label{note:jordan_decomposition_of_a_function}
...
\begin{thm}[Properties of Functions of Bounded Variation]\label{thm:properties_of_functions_of_bounded_variation}
  \begin{enumerate}
...
\begin{defn}[Absolutely Continuous Functions]\index{Absolutely Continuous Function}\label{defn:absolutely_continuous_functions}
  A function $f : \mathbb{R} \to \mathbb{C}$ is said to be
  \hlnoteb{absolutely continuous} if $\forall \epsilon > 0$, $\exists \delta > 0$
...
% section functions_of_bounded_variation_continued (end)

% chapter lecture_28_nov_15th_2019 (end)

\chapter{Lecture 29 Nov 18th 2019}%
\label{chp:lecture_29_nov_18th_2019}
% chapter lecture_29_nov_18th_2019

\section{Functions of Bounded Variation (Continued 2)}%
\label{sec:functions_of_bounded_variation_continued_2}
% section functions_of_bounded_variation_continued_2

\begin{propo}[Measure Constructed From A Bounded, Absolutely Continuous, Increasing, Right-Continuous Function]\label{propo:measure_constructed_from_a_bounded_absolutely_continuous_increasing_right_continuous_function}
...
\begin{lemma}[Absolutely Continuous Functions are of Bounded Variation]\label{lemma:absolutely_continuous_functions_are_of_bounded_variation}
...
\begin{lemma}[Total Variation of Absolutely Continuous Functions are Absolutely Continuous]\label{lemma:total_variation_of_absolutely_continuous_functions_are_absolutely_continuous}
...
\begin{lemma}[Building Block for Fundamental Theorem of Calculus for General Measures]\label{lemma:building_block_for_fundamental_theorem_of_calculus_for_general_measures}
...
% section functions_of_bounded_variation_continued_2 (end)

% chapter lecture_29_nov_18th_2019 (end)

\chapter{Lecture 30 Nov 20th 2019}%
\label{chp:lecture_30_nov_20th_2019}
% chapter lecture_30_nov_20th_2019

\section{Functions of Bounded Variation (Continued 3)}%
\label{sec:functions_of_bounded_variation_continued_3}
% section functions_of_bounded_variation_continued_3

\begin{thm}[Anti-Derivative of Bounded Integrable Functions]\label{thm:anti_derivative_of_bounded_integrable_functions}
...
\begin{thm}[Fundamental Theorem of Calculus (Lebesgue Version)]\index{Fundamental Theorem of Calculus (Lebesgue Version)}\label{thm:fundamental_theorem_of_calculus_lebesgue_version}
...
\begin{proof}
  ...
  \hlbnoted{$(2) \implies (3)$} This is proven in
  \cref{thm:anti_derivative_of_bounded_integrable_functions}.
\end{proof}

% section functions_of_bounded_variation_continued_3 (end)

\section{The \texorpdfstring{$L^p$}{Lp}-spaces}%
\label{sec:the_l_p_spaces}
% section the_l_p_spaces

\marginnote{It is helpful to review some of the contents from real analysis on
metric spaces, particularly in
\begin{itemize}
  \item metric;
  \item convergence;
  \item Cauchy sequences; and
  \item completeness.
\end{itemize}
You may find the material at
\href{https://tex.japorized.ink/PMATH351F18/classnotes.pdf}{PMATH 351}.

It may also be helpful to review some of the materials on $L^p$ spaces
in the course on Lebesgue measure and integration
(\href{https://tex.japorized.ink/PMATH450/classnotes.pdf}{PMATH 450}).
}

\begin{defn}[$\mathcal{L}^p$-spaces]\index{$\mathcal{L}^p$-spaces}\label{defn:_mathcal_l_p_spaces}
...
\begin{defn}[Essentially Bounded]\index{Essentially Bounded}\label{defn:essentially_bounded}
  Let $f : X \to \mathbb{R}_e$ be a measurable function.
  We say that $f$ is \hlnoteb{essentially bounded} if
  \begin{equation*}
    \exists M \quad \mu (\{ x : \abs{f(x)} > M \}) = 0.
  \end{equation*}
\end{defn}

\begin{defn}[Essential Norm]\index{Essential Norm}\label{defn:essential_norm}
  Let $f : X \to \mathbb{R}_e$ be a measurable function.
  We define the \hlnoteb{essential norm} of $f$ as
  \begin{equation*}
...
\begin{note}
  While we call $\norm{f}_{\infty}$ the essential \hlnoteb{norm},
  we have not defined what a norm is.
  We shall do that later.
\end{note}

\begin{defn}[$\mathcal{L}^{\infty}$-space]\index{$\mathcal{L}^{\infty}$-space}\label{defn:_mathcal_l_infty_space}
...
\begin{defn}[Hölder conjugates]\index{Hölder conjugates}\label{defn:holder_conjugates}
  Let $1 \leq p \leq \infty$ and $1 \leq q \leq \infty$.
  We say that $(p, q)$ is a \hlnoteb{Hölder conjugate} if
  \begin{equation*}
    \frac{1}{p} + \frac{1}{q} = 1,
  \end{equation*}
  where we set
...
\begin{thm}[Hölder's Inequality]\index{Hölder's Inequality}\label{thm:holder_s_inequality}
...
% chapter lecture_30_nov_20th_2019 (end)

\chapter{Lecture 31 Nov 22th 2019}%
\label{chp:lecture_31_nov_22th_2019}
% chapter lecture_31_nov_22th_2019

\section{The \texorpdfstring{$L^p$}{Lp}-spaces (Continued)}%
\label{sec:the_l_p_spaces_continued}
% section the_l_p_spaces_continued

\begin{propo}[$\mathcal{L}^p$-spaces are Vector Spaces]\label{propo:_cal_l_p_spaces_are_vector_spaces}
  Let $1 \leq p \leq \infty$.
  $\mathcal{L}^p(X, \mathfrak{M}, \mu)$ is a vector space.
\end{propo}

\begin{proof}
  \hlbnotec{Case: $1 < p < \infty$}
  Let $\alpha \in \mathbb{R}$ and $f, g \in \mathcal{L}^p$.

  \noindent
  \hlbnoted{Closure under scalar multiplication}
  Notice that
  \begin{equation*}
  ...
  \noindent
  \hlbnoted{Closure under addition}
  First, notice that
  \begin{equation*}
...
\begin{proof}
  ...
  Then, using \nameref{thm:holder_s_inequality} at $(*)$,
  we have
  \begin{align*}
    \norm{f + g}_p^p 
...
\begin{defn}[Norm]\index{Norm}\label{defn:norm}
  Let $\mathcal{V}$ be a vector space over $\mathbb{C}$.
  We call the function $\norm{\cdot} : \mathcal{V} \to \mathbb{R}$ a \hlnoteb{norm}
...
\begin{defn}[Normed Space]\index{Normed Space}\label{defn:normed_space}
  Let $\mathcal{V}$ be a vector space over $\mathbb{C}$
  and $\norm{\cdot}$ be a norm on $\mathcal{V}$.
  We call the pair $(\mathcal{V}, \norm{\cdot})$ a \hlnoteb{normed space}.
\end{defn}
...
\begin{note}
  ...
  \sidenote{We shall not show that $\quotient{\mathcal{V}}{\mathcal{N}}$
  is a vector space, but this is not hard, and those that are familiar with
  vector spaces will quickly realize so.}
  We can show that $\quotient{\mathcal{V}}{\mathcal{N}}$ is a vector space.
  We may then set
  \begin{equation*}
    \norm{[v]} \coloneqq \norm{v},
  \end{equation*}
  which will then be a \hlnotec{norm} on $\quotient{\mathcal{V}}{\mathcal{N}}$.
...
\begin{defn}[$L^p$-spaces]\index{$L^p$-spaces}\label{defn:_l_p_spaces}
  Let $(X, \mathfrak{M}, \mu)$ be a measure space.
  Let $1 \leq p \leq \infty$.
...
\begin{note}
  We define the \hlnotec{norm} of $L^p$-spaces 
  as we do in the last note, i.e. we define
  \begin{equation*}
    \norm{[f]}_p \coloneqq \norm{f}_p
  \end{equation*}
  for any $[f] \in L^p(X, \mathfrak{M}, \mu)$.
  For $1 \leq p < \infty$, \nameref{thm:minkowski_s_inequality}
...
\begin{remark}[Normed spaces are metric spaces]
  It is important that we note the following.
  \sidenote{This requires knowledge from metric spaces. See
  \href{https://tex.japorized.ink/PMATH351F18/classnotes.pdf}{PMATH 351}.}
  A norm \hlnotea{induces} a metric,
...
\begin{defn}[Banach Space]\index{Banach Space}\label{defn:banach_space}
  ...
  \hlnoteb{Banach space} if $(\mathcal{V}, \rho)$ is complete,
...
\begin{thm}[Riesz-Fischer Theorem]\index{Riesz-Fischer Theorem}\label{thm:riesz_fischer_theorem}
  Let $(X, \mathfrak{M}, \mu)$ be a measure space. For any $1 \leq p \leq \infty$,
  $(L^p(X, \mathfrak{M}, \mu), \norm{\cdot}_p)$ is a Banach space.
\end{thm}

\begin{proof}
  \hlbnotec{Case: $1 \leq p < \infty$}
  ...
  \end{equation*}
  \sidenote{Note that we may drop the absolute value on $h_m(x)$
  since $h_m(x) \geq 0$.}
  Furthermore, we know that $h_m(x)^p \nearrow h(x)^p$.
  By the \nameref{thm:monotone_convergence_theorem}, we have
  \begin{equation*}
    \int_{X} h(x)^p \dif{\mu}
...
\begin{thm}[Density of Simple Functions in $\mathcal{L}^p$]\label{thm:density_of_simple_functions_in_cal_p}
...
\begin{defn}[Bounded Linear Functional]\index{Bounded Linear Functional}\label{defn:bounded_linear_functional}
  Let $(\mathcal{V}, \norm{\cdot})$ be a normed vector space.
  A linear function $T : \mathcal{V} \to \mathbb{R}$ is called
  a \hlnoteb{linear functional}.
  It is called a \hlnoteb{bounded linear functional}
...
\begin{propo}[Bounded Linear Functionals are Continuous]\label{propo:bounded_linear_functionals_are_continuous}
  Let $T : \mathcal{V} \to \mathbb{R}$ be a bounded linear functional. Then
  $T$ is continuous. \sidenote{The converse is also true,
  and we have seen this in
  \href{https://tex.japorized.ink/PMATH351F18/classnotes.pdf}{PMATH 351} and
  \href{https://tex.japorized/ink/PMATH450/classnotes.pdf}{PMATH 450}.}
\end{propo}
...
\begin{propo}[$p$-norms As Bounded Linear Functionals]\label{propo:_p_norms_as_bounded_linear_functionals}
  Let $(p, q)$ be a Hölder conjugate,
...
\begin{proof}
  ...
  By \nameref{thm:holder_s_inequality},
  ...
  \sidenote{We consider one example where the other inequality is true,
  and this is sufficient to show that the other inequality is true.}
...
\begin{thm}[Riesz Representation Theorem]\index{Riesz Representation Theorem}\label{thm:riesz_representation_theorem}
  Let $1 \leq p < \infty$
...
\begin{proof}
  \hlbnoted{Case: $1 < p < \infty$}

  \noindent
  \hlbnotec{Case: $\mu(X) < \infty$}
  ...
  \noindent
  \hlbnoteb{Claim: $\nu$ is a finite signed measure} \hlwarn{To be proven}

  \noindent
  \hlbnotea{Constructing $g \in \mathcal{L}^q$}
  ...
  \noindent
  \hlbnotec{Case: $\mu$ is $\sigma$-finite (sketch proof)}
...
\begin{remark}
  \begin{enumerate}
    \item Note that \nameref{thm:riesz_representation_theorem} is false
...
% section the_l_p_spaces_continued_2 (end)

% chapter lecture_32_nov_25th_2019 (end)
