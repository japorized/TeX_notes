...
\begin{note}
  Recall that for a group $G$ and $g \in G$, we have $g^0 = 1$, $g^1 = g$, and $\left(g^{-1}\right)^{-1} = g$. Thus for addition, we have\sidenote{Note that the first $0$ is an integer while the second $0$ is a zero in $R$.}
...
\begin{enumerate}
  \item $0, 1 \in S$ \sidenote{The $0 \in S$ is certainly not necessary to be shown, since from part $(2)$ we would have $s \in S \implies 0 \in ( s - s ) \in S$.}
