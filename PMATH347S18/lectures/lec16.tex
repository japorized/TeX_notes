...
\end{ex}
}, which is the equivalent statement of \hyperref[thm:cayley_s_theorem]{Cayley's Theorem}\index{Cayley's Theorem}.
...
\label{thm:orbit_decomposition_theorem}
  ...
  \begin{align*}
    x \in X_f &\iff \forall a \in G \enspace a \cdot x = x \\
      &\iff \forall g \cdot x \in G \cdot x \quad g \cdot x = x \\
      &\iff \abs{G \cdot x} = 1
  \end{align*}}
