...
\begin{remark}
  By \cref{thm:downward_lowenheim_skolem}, $(\mathbb{R}, 0, +, -)$ has many proper elementary subgroups, e.g. $\mathbb{Q}$.\sidenote{\hlwarn{How so? Is it simply by taking $q + r$ for $q \in \mathbb{Q}$ and $r \in \mathbb{R} \setminus \mathbb{Q}$.}}
\end{remark}
...
\begin{eg}
  Let $\mathcal{L} = \{ e, \cdot, \inv \}$. The following classes is elementary:\sidenote{\hlwarn{How do we know this?}}
  \begin{itemize}
    \item groups
    \item abelian groups
...
\begin{lemma}[Equivalence to a Complete Theory]\label{lemma:equivalence_to_a_complete_theory}
  ...
  \begin{enumerate}
    ...
    \item $\bar{T} = \Th(\mathcal{M})$ for some (equivalently any)\sidenote{What does this mean?} $\mathcal{M} \models T$;
...
\begin{proof}
  ...
  \end{equation*}
  Let $b \in N$ satisfy this formula\sidenote{During lecture, it was mentioned that $b$ should not satisfy other properties. \hlwarn{What are these other properties, and why should $b$ not satisfy these other properties?}} and define $f' : \{ a_1, \ldots, a_k, a_{k + 1} \} \to N$ such that 
  \begin{gather*}
...
\begin{eg}
  \begin{enumerate}
    ...
    \item Let $I \in Set$, and $\kappa \leq \abs{I} \in \Card$. Then
      ...
      \end{equation*}
      is a filter. In particular, if $I = \omega$, and $\kappa = \aleph_0$. Then the above filter, which is the set of \hlnotea{cofinite sets}, is called a \hldefn{Frechet filter}.
