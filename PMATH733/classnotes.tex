...
\begin{solution}
  \sidenote{Question: why did we look at the automorphisms? \textbf{Answer:} By \cref{crly:automorphisms_on_b_definable_sets}, automorphisms that fixes, in this case, $\emptyset$ pointwise, which is trivially true, should remap the defined set into an element of its own. We use this fact to obtain a contradiction to show that addition is not definable.}We need make this inference from automorphisms on $\mathcal{Q}$. Let $\sigma : \mathbb{Q} \to \mathbb{Q}$ be an automorphism. Then we have that
  \begin{gather*}
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \draw[->] (0, -2.5) -- (0, 2.5) node[above] {$\phi(x)$};
      \draw[-] (-1,-1) -- (1, 1);
      \draw[-] (-1,-1) -- (-1.5,-2.5);
      \draw[-] (1,1) -- (1.5,2.5);
    \end{tikzpicture}
...
\begin{thm}[Downward L\"{o}wenheim-Skolem]\index{Downward L\"{o}wenheim-Skolem}\label{thm:downward_lowenheim_skolem}
...
\begin{proof}
  ...
  Given an $A_n$, we shall show a way to construct $A_{n + 1}$. \todo{\hlwarn{Requires clarification}} 
\end{proof}

\begin{eg}
  Consider the language $\mathcal{L}$ of rings and the $\mathcal{L}$-structure $\mathcal{C} = (\mathbb{C}, 0, 1, +, -, \times)$. We know that $\mathcal{Q} = (\mathbb{Q}, 0, 1, +, -, \times)$ is an $\mathcal{L}$-substructure of $\mathcal{C}$. It can be shown that the structure that is the \hlnotea{algebraic closure} of $\mathbb{Q}$ is an elementary $\mathcal{L}$-substructure of $\mathcal{C}$.\sidenote{\todo{\hlwarn{Why? How can we show this?}}}
\end{eg}
...
\begin{proof}
  ...
  \end{gather*}
  So $G$ must be some non-trivial divisible subgroup of $\mathcal{Q}$. Let $\frac{n}{m} \in G$, with $n \neq 0$. WMA\sidenote{Short for \hlnotea{We May Assume}. We may indeed assume that both $n$ and $m$ are strictly positive, or we can just factor out $-1$.} $n, m > 0$. Then $n \in G$ by $n$ divisibility in $G$. Thus $1 \in G$ Thus $\mathbb{Z} \leq G \leq \mathbb{Q}$, and so\sidenote{\todo{\hlwarn{How does this follow?}}} $G = \mathbb{Q}$ by divisibility.
\end{proof}

\begin{remark}
  By \hyperref[thm:downward_lowenheim_skolem]{Downward L\"{o}wenheim-Skolem}, $(\mathbb{R}, 0, +, -)$ has many proper elementary subgroups, e.g. $\mathbb{Q}$.\sidenote{\todo{\hlwarn{How so? Is it simply by taking $q + r$ for $q \in \mathbb{Q}$ and $r \in \mathbb{R} \setminus \mathbb{Q}$.}}}
\end{remark}
...
Fortunately, just so that we can have some sort of closure\sidenote{Not in a mathematical sense, well perhaps.}, we shall present the following theorem without proof.
...
% chapter lecture_16_nov_01st

\section{First-order Logic (Continued 6)}%
\label{sec:first_order_logic_continued_6}
% section first_order_logic_continued_6

\subsection{Parameters and Definable Sets (Continued)}%
\label{sub:parameters_and_definable_sets_continued}
% subsection parameters_and_definable_sets_continued

\begin{crly}[Automorphisms on $B$-definable Sets]\label{crly:automorphisms_on_b_definable_sets}
...
% subsection parameters_and_definable_sets_continued (end)

\subsection{Theories}%
\label{sub:theories}
% subsection theories

\begin{defn}[$\mathcal{L}$-Theory]\index{$\mathcal{L}$-Theory}\label{defn:_l_theory}
  An \hlnoteb{$\mathcal{L}$-Theory} is simply a set of $\mathcal{L}$-sentences.
\end{defn}

\begin{defn}[Model]\index{Model}\label{defn:model}
  A \hlnoteb{model} of an $\mathcal{L}$-theory $T$ is an $\mathcal{L}$-structure $\mathcal{M}$ such that $\mathcal{M} \models \sigma$ for all $\sigma \in T$. We denote this by $\mathcal{M} \models T$. A theory is said to be \hldefn{consistent} if it has a model.
\end{defn}

\begin{defn}[$\mathcal{L}$-elementary / $\mathcal{L}$-axiomatizable]\index{$\mathcal{L}$-elementary}\index{$\mathcal{L}$-axiomatizable}\label{defn:l_axiomatizable}
  A class $K$ of $\mathcal{L}$-structures is said to be \hlnoteb{$\mathcal{L}$-elementary} (or \hlnoteb{$\mathcal{L}$-axiomatizble}) if there is an $\mathcal{L}$-theory $T$ such that
  \begin{equation*}
...
\begin{eg}
  Let $\mathcal{L} = \{ e, \cdot, \inv \}$. The following classes are elementary:\sidenote{\todo{\hlwarn{How do we know this?}}}
  \begin{itemize}
    \item groups
    \item abelian groups
...
\begin{defn}[Theories of a Structure]\label{defn:theories_of_a_structure}
...
\begin{defn}[$\mathcal{L}$-elementarily Equivalent]\index{$\mathcal{L}$-elementarily Equivalent}\label{defn:_l_elementarily_equivalent}
  Given two $\mathcal{L}$-structures $\mathcal{M}$ and $\mathcal{N}$, we sya that $\mathcal{M}$ is \hlnoteb{$\mathcal{L}$-elementarily equivalent} to $\mathcal{N}$ if
  \begin{equation*}
...
\begin{defn}[Implication]\index{Implication}\label{defn:implication}
  Suppose $T$ is an $\mathcal{L}$-theory, and $\sigma$ an $\mathcal{L}$-sentence. We say that $T$ \hlnoteb{implies} $\sigma$ (or $T$ \hlnoteb{entails} $\sigma$), denoted by $T \models \sigma$, if for every $\mathcal{M} \models T$, we have $\mathcal{M} \models \sigma$.
\end{defn}

\begin{defn}[Complete]\index{Complete}\label{defn:complete}
...
\begin{lemma}[Equivalence to a Complete Theory]\label{lemma:equivalence_to_a_complete_theory}
  Suppose $T$ is a consistent $\mathcal{L}$-theory. TFAE
  \begin{enumerate}
    \item $T$ is complete;
    \item $\bar{T} := \{ \sigma \mid T \models \sigma \}$, called \hldefn{the set of consequences of $T$}, is maximally consistent;
    \item $\bar{T} = \Th(\mathcal{M})$ for some (equivalently any)\sidenote{\todo{What does this mean?}} $\mathcal{M} \models T$;
    \item Any $2$ models of $T$ are elementarily equivalent.
  \end{enumerate}
\end{lemma}

\begin{proof}
  \hlbnoted{$(1) \implies (2)$}: Suppose $\bar{T}$ is not maximal, i.e. $\bar{T} \subsetneq S$  for some $S$ an $\mathcal{L}$-theory. Let $\sigma \in S \setminus \bar{T}$. Then $T \not\models \sigma$. But $T$ is complete, and so $T \models \neg \sigma$. Thus $\neg \sigma \in \bar{T} \subsetneq S$. Then we have that $\{ \sigma, \neg \sigma \} \subset S$ and so $S$ has no models, i.e. $S$ is inconsistent.

  \noindent\hlbnoted{$(2) \implies (3)$}: If $\mathcal{M}$ is a model of $T$, then $\mathcal{M}$ is a model of $\bar{T}$, thus $\bar{T} \subseteq \Th(\mathcal{M})$. Since $\bar{T}$ is maximal, we must have $\bar{T} = \Th(\mathcal{M})$.

  \noindent\hlbnoted{$(3) \implies (4)$}: Suppose $\mathcal{M} \models T$ such that $\bar{T} = \Th(\mathcal{M})$. Let $\mathcal{N} \models T$. Then $\mathcal{N} \models \bar{T}$, and as above, we have $\bar{T} = \Th(\mathcal{N})$. Therefore $\Th(\mathcal{M}) = \Th(\mathcal{N})$, i.e. $\mathcal{M} \equiv \mathcal{N}$.

  \noindent\hlbnoted{$(4) \implies (1)$}: Suppose $T \not\models \sigma$, where $\sigma$ is an $\mathcal{L}$-sentence. Then there exists $\mathcal{M} \models T$ such that $\mathcal{M} \models \neg \sigma$. Let $\mathcal{N} \models T$. By assumption, since any two models of $T$ are elementarily equivalent, we must also have $\mathcal{N} \models \neg \sigma$. Therefore $T \models \neg \sigma$.\qed\
\end{proof}

% subsection theories (end)

% section first_order_logic_continued_6 (end)

% chapter lecture_16_nov_01st (end)

\chapter{Lecture 17 Nov 06th}%
\label{chp:lecture_17_nov_06th}
% chapter lecture_17_nov_06th

\section{First-order Logic (Continued 7)}%
\label{sec:first_order_logic_continued_7}
% section first_order_logic_continued_7

\subsection{Theories (Continued)}%
\label{sub:theories_continued}
% subsection theories_continued

\begin{defn}[Partial $\mathcal{L}$-elementary Map]\index{Partial $\mathcal{L}$-elementary Map}\label{defn:partial_l_elementary_map}
  Let $\mathcal{M}$ and $\mathcal{N}$ be $\mathcal{L}$-structures, $A \subseteq M$, and $f : A \to N$ a function. $f$ is a \hlnoteb{partial $\mathcal{L}$-elementary map} (pem) if for all $\mathcal{L}$-formulae $\phi(x_1, \ldots, x_n)$, and for all $a_1, \ldots, a_n \in A$,
  \begin{equation*}
...
\begin{propo}[Elementary Equivalence in Finite Stuctures]\label{propo:elementary_equivalence_in_finite_stuctures}
...
\begin{proof}
  ...
  \end{equation*}
  Let $b \in N$ satisfy this formula\sidenote{During lecture, it was mentioned that $b$ should not satisfy other properties. \todo{\hlwarn{What are these other properties, and why should $b$ not satisfy these other properties?}}} and define $f' : \{ a_1, \ldots, a_k, a_{k + 1} \} \to N$ such that 
  \begin{gather*}
...
% subsection theories_continued (end)

% section first_order_logic_continued_7 (end)

\section{Compactness}%
\label{sec:compactness}
% section compactness

\begin{thmnonum}[Compactness Theorem]\index{Compactness Theorem}
...
\begin{crly}[Corollary of Compactness Theorem]\label{crly:corollary_of_compactness_theorem}
...
In this course, we shall study a different proof for the Compactness Theorem, one that uses \hlnotea{ultraproducts}.

\subsection{A proof of compactness using ultraproducts}%
\label{sub:a_proof_of_compactness_using_ultraproducts}
% subsection a_proof_of_compactness_using_ultraproducts
...
\begin{defn}[Filter]\index{Filter}\label{defn:filter}
...
\begin{eg}
  \begin{enumerate}
    ...
    \item Let $I \in Set$, and $\kappa \leq \abs{I} \in \Card$. Then
      ...
      \end{equation*}
      is a filter. In particular, if $I = \omega$, and $\kappa = \aleph_0$. Then the above filter, which is the set of \hlnotea{cofinite sets}, is called a \hldefn{Fr\'{e}chet filter}.
    \item Let $I \in \Set \setminus \{\emptyset\}$, and $a \in I$. The set
      \begin{equation*}
        \mathcal{F}_a = \{ A \subseteq I \mid a \in A \}
      \end{equation*}
      is called a \hldefn{principal filter} on $I$.
  \end{enumerate}
\end{eg}

\begin{defn}[Ultrafilter]\index{Ultrafilter}\label{defn:ultrafilter}
  An \hlnoteb{ultrafilter} on $I$ is a filter that is maximal.
\end{defn}
...
\begin{lemma}[Existence of Ultrafilters]\label{lemma:existence_of_ultrafilters}
...
\begin{lemma}[Equivalent Characterization of an Ultrafilter]\label{lemma:equivalent_characterization_of_an_ultrafilter}
...
We are now ready to define what an \hlnotea{ultraproduct} is.

\begin{defn}[Ultraproduct]\index{Ultraproduct}\label{defn:ultraproduct}
  Let $(\mathcal{M}_i : i \in I)$ be a sequence of $\mathcal{L}$-structures indexed by $I \neq \emptyset$, and $\mathcal{U}$ an ultrafilter. An \hlnoteb{ultrafilter} is an $\mathcal{L}$-structure which is defined as follows: we denote the ultraproduct $\mathcal{M}$ as
  \begin{equation*}
...
% subsection a_proof_of_compactness_using_ultraproducts (end)

% section compactness (end)

% chapter lecture_17_nov_06th (end)

\chapter{Lecture 18 Nov 08th}%
\label{chp:lecture_18_nov_08th}
% chapter lecture_18_nov_08th
...
% subsection a_proof_of_compactness_using_ultraproducts_continued
...
\begin{eg}
  ...
  \end{equation*}
  but neither $(1, 0, 1, 0, \ldots)$ nor $(0, 1, 0, 1, \ldots)$ are $(0, 0, 0, 0, \ldots)$. Therefore $R$ is not an \hlnotea{integral domain}.
...
\begin{thm}[{\L}o\'{s}]\index{{\L}o\'{s}}\label{thm:los}
...
\begin{proof}
  ...
  \noindent
  \hlbnotea{Claim} If $t(x_1, \ldots, x_n)$ is an $\mathcal{L}$-term, then for any $[\alpha_1], \ldots, [\alpha_n] \in \mathcal{M}$, we have
  \begin{equation*}
    t^\mathcal{M} ( [ \alpha_1 ], \ldots, [ \alpha_n ] ) = [ ( t^{\mathcal{M}_i} ( \alpha_1(i), \ldots, \alpha_n(i) ) : i \in I ) ].
  \end{equation*}

  \noindent
  \hlbnoteb{Proof of claim} This is clearly true for when our $\mathcal{L}$-terms is either constants for functions, simply from the way of which we have defined them. If our $\mathcal{L}$-term is a variable, i.e. if $t(x_i) = x_i$, then the result follows as the $x_i$'s are variables for exactly any of the $[\alpha_j]$'s. $\dashv$

  \noindent
  We shall prove {\L}o\'{s}'s theorem by the complexity of $\phi(x_1, \ldots, x_n)$.

  \noindent
  \hlbnoted{Atomic formulae} If $\phi$ is of the form
  \begin{equation*}
    t(x_1, \ldots, x_n) = s (x_1, \ldots, x_n),
  \end{equation*}
  then we have
  ...
  where the second $\iff$ is by the interpretation of $R$ in $\mathcal{M}$.
    ...
    \noindent
    \hlbnoted{Quantifiers} As mentioned in a few of the previous proofs in earlier propositions and theorems, it suffices to only prove for the existential case. Suppose $\phi$ is of the form $\exists y \psi(y)$, for some $\mathcal{L}$-formula $\psi(x_1, \ldots, x_n, y)$. Then
    \begin{align*}
      &\mathcal{M} \models \phi([\alpha_1], \ldots, [\alpha_n]) \\
...
% subsection a_proof_of_compactness_using_ultraproducts_continued (end)

% section compactness_continued (end)

% chapter lecture_18_nov_08th (end)

\chapter{Lecture 19 Nov 13th}%
\label{chp:lecture_19_nov_13th}
% chapter lecture_19_nov_13th
...
% subsection a_proof_of_comapactness_using_ultraproducts_continued_2

We shall introduce one of the special cases of an ultraproduct:

\begin{defn}[Ultrapower]\index{Ultrapower}\label{defn:ultrapower}
  ...
  \end{equation*}
  is called the \hlnoteb{ultrapower} of $\mathcal{M}$.
\end{defn}

\begin{crly}[Corollary of {\L}o\'{s}]\index{Corollary of {\L}o\'{s}}\label{crly:corollary_of_los}
...
\begin{note}
  ...
  Also, if $\mathcal{U}$ is principal, then $\mathcal{M} \simeq \prod_{\mathcal{U}} \mathcal{M}$ \sidenote{This may be an interesting or at least a relatively simple exercise.
  \begin{ex}
    Show that if $\mathcal{U}$ is principal in \cref{crly:corollary_of_los}, then $\mathcal{M} \simeq \prod_{\mathcal{U}} \mathcal{M}$.
  \end{ex}}
\end{note}
...
\begin{defn}[$\aleph_1$-Compact]\index{$\aleph_1$-Compact}\label{defn:_aleph_1_compact}
  We say that a set $A$ is \hlnoteb{$\aleph_1$-compact} if any countable collection of definable non-empty sets in $A$, with the \hlnotea{finite intersection property}\sidenote{The finite intersection property is such that every finite non-empty subcollection non-empty sets has a non-empty intersection. See also \href{https://tex.japorized.ink/PMATH351F18/classnotes.pdf}{notes on PMATH 351 (Real Analysis)}.}, then the collection has a non-empty intersection.
\end{defn}

\begin{propo}[Ultraproducts with a Non-Principal Ultrafilter is $\aleph_1$-compact]\label{propo:ultraproducts_with_a_non_principal_ultrafilter_is_aleph_1_compact}
...
\begin{note}
  \hlwarn{The given proof is not one that I can fully agree with}, since in the proof we may somehow assume that the sets are nested, which is not given in our assumption, and may not actually be a necessary condition. The provided proof will be recorded for archiving purposes. 
\end{note}

\begin{proof}
  Let $\mathcal{M} = \prod_{\mathcal{U}} \mathcal{M}_i$, and $(F_i : i < \omega)$ a sequence of definable non-empty sets in $\mathcal{M}$, such that $F_i \neq \emptyset$. \hlwarn{Suppose that $F_j \subseteq F_i$ for $i < j$}. We may assume that $F_0 = M^n$, given by the $\mathcal{L}$-formula $x = x$. For $0 < i < \omega$, let $\phi_i(x)$ be the $\mathcal{L}$-formula that defines $F_i$. For each $i$, let
  \begin{equation*}
  ...
  \end{equation*}
  \todo{Come back to this proof after the exam.} \marginnote{Leaving it behind for now to focus on the more important stuff. See also \url{https://math.stackexchange.com/questions/3027612/ultraproducts-with-a-non-principal-ultrafilter-is-aleph-one-compact} [solved].}
\end{proof}
...
\begin{thm}[Compactness Theorem]\index{Compactness Theorem}\label{thm:compactness_theorem}
...
\begin{proof}
  ...
  \end{equation*}
  The collection of these $X_i$'s gives us the structure of a \hlnotea{lattice}:
  \begin{marginfigure}
    \begin{tikzcd}
     &  &  &  &  &  &  \\
     &  &  & i \cup j \arrow[lu, no head, color=be-red] \arrow[ru, no head, color=be-red] &  &  &  \\
     &  & i \arrow[ru, no head, color=be-red] \arrow[lluu, no head] &  & j \arrow[lu, no head, color=be-red] \arrow[rruu, no head] &  &  \\
     &  &  & i \cap j \arrow[lu, no head] \arrow[ru, no head] &  &  &  \\
     &  &  & \emptyset \arrow[u, no head] &  &  & 
    \end{tikzcd}
  ...
  \end{marginfigure}
  \sidenote{Now if we simply consider the collection of these collections:
  \begin{equation*}
    \mathcal{V} = \{ X_i : i \in I \},
  \end{equation*}
  we notice that $\mathcal{V}$ is almost a filter, but not quite: we have
  \begin{itemize}
    \item $I = X_\emptyset \in \mathcal{V}$ and $\emptyset \notin \mathcal{V}$; and
    \item for $X_i, X_j \in \mathcal{V}$, we have $X_i \cap X_j = X_{i \cup j} \in \mathcal{V}$.
  \end{itemize}
  \todo{Need to show an example of $\mathcal{V}$ failing to ``capture'' supersets.}}
  Consider
  \begin{equation*}
  ...
  \end{itemize}
  Let $\mathcal{U} \supset \mathcal{F}$ be an ultrafilter\sidenote{ \begin{ex} Show that $\mathcal{U}$ is a non-principal ultrafilter.  \end{ex}}. Let $\mathcal{M} = \prod_{\mathcal{U}} M_i$.
  ...
  \end{equation*}
  Thus by \hyperref[thm:los]{\text{{\L}o\'{s}}} we have $\mathcal{M} \models \sigma$ and so $\mathcal{M} \models T$ as required.\qed\
\end{proof}

% subsection a_proof_of_comapactness_using_ultraproducts_continued_2 (end)

% section compactness_continued_2 (end)

% chapter lecture_19_nov_13th (end)

\chapter{Lecture 20 Nov 15th}%
\label{chp:lecture_20_nov_15th}
% chapter lecture_20_nov_15th
...
% subsection first_applications_of_compactness
...
% subsection first_applications_of_compactness (end)

% section compactness_continued_3 (end)

% chapter lecture_20_nov_15th (end)
