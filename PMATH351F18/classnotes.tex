...
\begin{eg}
  ...
  \begin{equation*}
    x_n \to x_0 \iff \exists N \in \mathbb{N} \; \foral ln \geq N \; x_n = x_0.
  \end{equation*}
\end{eg}
...
Note, however, that $c_00 \subset \ell_1 \subset c_0$ is not closed. Also $\ell_p$ is not closed in $c_0$.
...
\begin{thm}[Sequential Characterization of Continuity]\index{Sequential Characterization of Continuity}\label{thm:sequential_characterization_of_continuity}
  For metric spaces $(X, d_X)$ and $(Y, d_Y)$, and $f : X \to Y$, TFAE
  \begin{enumerate}
    \item $f$ is continuous as $x_0 \in X$;
...
\begin{proof}
  ...
  $\implies \forall n \in \mathbb{N} \; \exists x_n \in B_X\left(x_0, \frac{1}{n}\right) \implies f(x_n) \notin B_Y(f(x_0), \epsilon)$ \\
