...
\begin{proof}
  Let $\epsilon > 0$. Since $\mathcal{F}$ is equicontinuous, for each $x_0 \in X$, we can find $\delta_{x_0} > 0$ if $x \in B(x_0, \delta_{x_0})$, then $\abs{ f(x) - f(x_0) } < \frac{\epsilon}{2}$ for any $f \in \mathcal{F}$. Since $(X, d)$ is compact, the cover $\{ B(x_0, \delta_{x_0}) \}_{x_0 \in X}$ has a \hyperref[thm:lesbesgue]{Lesbesgue Number} $\delta_0 > 0$. Then, let $0 < \delta < \delta_0$. If for $w , z \in X$ we have $d(w, z) < \delta$, then $z \in B(w, \delta) \subset B(x_0^', \delta_{x_0'})$ for some $x_0' \in X$. Then
  \begin{equation*}
