...
\begin{proof}
  Assume that $f_n \to f_0$ on $(a, b)$, pointwise. \hlwarn{Need clarification on why the claim uses the fact that the sequence $\{ f_n(x^*) \}$ is Cauchy. We only have pointwise convergence.} 
\end{proof}
...
\begin{crly}[Continuity of the Limit of a Sequence of Pointwise Convergent Functions on a Residual Set]\label{crly:continuity_of_the_limit_of_a_sequence_of_pointwise_convergent_functions_on_a_residual_set}
  Let $\{ f_n \} \subset C[a, b]$ be such that $f_n \to f_0$ pointwise on $[a, b]$. Then there exists a residual set $A \subset [a, b]$ such taht $f_0(x)$ is continuous at each $x \in A$.
\end{crly}
...
\begin{proof}
  ...
  \end{equation*}
  and so $\exists x_1 \in B \left[ x_0, \frac{1}{n_0} \right]$ such that $x_1 \in A$ but $x_0 \notin \bigcup_{n=1}^{N} U_n$. This contradicts the assumption that a subcover exists. But $A$ must have some subcover for we assumed that $A$ is compact. Therefor $A$ must be closed.
