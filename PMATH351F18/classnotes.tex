...
Given $A \subset (X, d)$ and $\epsilon > 0$. An \hlnoteb{$\epsilon$-net} for $A$ is a finite set $\{ x_1, \ldots, x_n \} \subset X$ such that
  \begin{equation*}
...
\begin{defn}[Totally Bounded]\index{Totally Bounded}\label{defn:totally_bounded}
  We say that a subset $A \subset (X, d)$ is \hlnoteb{totally bounded} if $A$ has an $\epsilon$-net for every $\epsilon > 0$.
\end{defn}
...
Note that boundedness and totally boundedness are \hlimpo{not equivalent}.
