...
\begin{proof}
  \begin{marginfigure}
    \begin{tikzpicture}
      \draw[fill,color=red] (0, 0) circle(2.2);
      \node at (0, 2.4) {$A_0$};
      \draw[fill,color=yellow] (0, 0) circle(1.8);
      \node[color=background] at (0, 2.0) {$\boldsymbol{A_1}$};
      \draw[fill,color=green] (0, 0) circle(1.4);
      \node[color=background] at (0, 1.6) {$\boldsymbol{A_2}$};
      \draw[fill,color=cyan] (0, 0) circle(1.0);
      \node[color=background] at (0, 1.2) {$\boldsymbol{A_3}$};
      \draw[fill,color=blue] (0, 0) circle(0.6);
      \node[color=background] at (0, 0.8) {$\boldsymbol{A_4}$};
      \draw[fill,color=foreground] (0, 0) circle(0.2);
      \node[color=background] at (0, 0.4) {$\boldsymbol{A_5}$};

      \draw[thick,->,color=background] (1.2,1.59705627484771405856) -- (0.9,0.8797958971132712) node[midway,left] {$\boldsymbol{f}$};
      \draw[thick,->,color=background] (1.2, 0) -- (0.4, 0) node[midway,below] {$\boldsymbol{f}$};
    \end{tikzpicture}
...
\end{equation*}
Notice that we would then have the following graph:
\begin{figure}[ht]
  \begin{center}
  \begin{tikzpicture}
    \draw[-latex'] (-3, 0) -- (3, 0);
...
\begin{proof}
  ...
  \begin{marginfigure}
    ...
    \draw[fill=blue!20!background] (1.5,1.5) -- (0, 1.5) -- (0, 0) -- (1.5, 0) -- cycle;
    \draw[domain=0:1.73205080756887729352] plot ( {\x},{\x^2} ) node[above] {$u = t^{ p - 1 }$};
    \draw[dotted] (1.5, 1.5) -- (0, 1.5) node[left] {$\beta$};
    \draw[dotted] (1.5, 1.5) -- (1.5, 0) node[below] {$\alpha$};
    \end{tikzpicture}
...
\begin{eg}[Cantor Set]\label{eg:cantor_set}
  ...
  \begin{figure}[ht]
    ...
    \begin{tikzpicture}
      ...
      \draw[{Arc Barb[length=5pt,width=15pt]}-{Arc Barb[length=5pt,width=15pt]},blue] (3,0) -- (6,0);
      \draw[{Arc Barb[length=5pt,width=15pt]}-{Arc Barb[length=5pt,width=15pt]},blue] (1,0) -- (2,0);
      \draw[{Arc Barb[length=5pt,width=15pt]}-{Arc Barb[length=5pt,width=15pt]},blue] (7,0) -- (8,0);

      \draw[{Bracket[length=5pt,width=15pt]}-{Bracket[length=5pt,width=15pt]},red] (0, 0) -- (1, 0);
      \draw[{Bracket[length=5pt,width=15pt]}-{Bracket[length=5pt,width=15pt]},red] (2, 0) -- (3, 0);
      \draw[{Bracket[length=5pt,width=15pt]}-{Bracket[length=5pt,width=15pt]},red] (6, 0) -- (7, 0);
      \draw[{Bracket[length=5pt,width=15pt]}-{Bracket[length=5pt,width=15pt]},red] (8, 0) -- (9, 0);
      
      \draw[{Arc Barb[length=5pt,width=15pt]}-{Arc Barb[length=5pt,width=15pt]},blue] (0.333333,0) -- (0.666667,0);
      \draw[{Arc Barb[length=5pt,width=15pt]}-{Arc Barb[length=5pt,width=15pt]},blue] (2.333333,0) -- (2.666667,0);
      \draw[{Arc Barb[length=5pt,width=15pt]}-{Arc Barb[length=5pt,width=15pt]},blue] (6.333333,0) -- (6.666667,0);
      \draw[{Arc Barb[length=5pt,width=15pt]}-{Arc Barb[length=5pt,width=15pt]},blue] (8.333333,0) -- (8.666667,0);
    \end{tikzpicture}
    \end{center}
...
\begin{defn}[Diameter of a Set]\index{Diameter}\label{defn:diameter_of_a_set}
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \draw[dashed,color=red] (1.4142135623730950488,1.4142135623730950488) -- (-1.4142135623730950488,-1.4142135623730950488);
      ...
      \node[draw,fill,circle,inner sep=1pt] at (0, -2) {};
      \draw[dashed,color=cyan] (2, 0) -- (0, -2);
      \draw[dashed,color=cyan] (1,1.73205080756887729352) -- (-1.73205080756887729352,-1);
    \end{tikzpicture}
...
\begin{thm}[Completeness of Finite Dimensional Normed Linear Spaces]\label{thm:completeness_of_finite_dimensional_normed_linear_spaces}
...
\end{equation*}
for any $f, g \in C[0, 1]$. Moreover, \hlimpo{$\Gamma(p)$ is a polynomial iff $p$ is a polynomial}.\sidenote{Basically, this part shows us that we can use $\phi$, which is also a continuous function, to scale the domain of $f$ so as to shrink it down to only at $[0, 1]$ instead of $[a, b]$.}
...
\end{equation*}
uniformly to within $\epsilon > 0$ \sidenote{\hlimpo{Notice} that if we rearrange the equation, we have
\begin{equation*}
  f(x) = g(x) + f(0) + x ( f(1) - f(0) )
\end{equation*}
which tells us that if we can approximate $g$ by a polynomial, then we can do so for $f$ cause the later term is also a polynomial.}, i.e.
\begin{equation*}
...
\begin{lemma}[Lemma for Weierstrass Approximation]\label{lemma:lemma_for_weierstrass_approximation}
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \draw[scale=1.7,domain=0:1] plot ({\x},{(1 - ( \x )^2)^(100)});
    \end{tikzpicture}
...
\begin{thm}[\vvimponote\ Weierstrass Approximation Theorem]\index{Weierstrass Approximation Theorem}\label{thm:weierstrass_approximation_theorem}
...
\begin{proof}
  ...
  \end{align*}
  \footnotetext{How did we arrive at this new limit of $\frac{1}{\sqrt{n}}$? \hlwarn{There is} no deep meaning behind the choice of $\frac{1}{\sqrt{n}}$. It's simply because it works.}
  and so we have
  \begin{equation*}
    c_n < \sqrt{n}.
  \end{equation*}
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \draw[scale=1.7,domain=-1:1] plot ({\x},{(1 - (\x)^2)^(100)});
      \draw[scale=1.7,domain=-1:1] plot ({\x},{(1 - (\x)^2)^(50)});
      \draw[scale=1.7,domain=-1:1] plot ({\x},{(1 - (\x)^2)^(10)});
    \end{tikzpicture}
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \draw[->] (0,-0.5) -- (0, 2) node[above] {$y$};
      \draw[scale=1.7,domain=-1:1] plot ({\x},{(1 - (\x)^2)^(10)});
      \draw[dashed] (-0.7,0.25) -- (-0.7,0) node[below] {$-\delta$};;
      \draw[dashed] (0.7,0.25) -- (0.7,0) node[below] {$\delta$};;
      \node[draw,circle,fill,inner sep=0.5pt,pin={[pin edge=latex'-, pin-distance=2pt]115:{$c_n\left(1 - \delta^2\right)^n$}}] at (-0.7, 0.25) {};
      \node[draw,circle,fill,inner sep=0.5pt,pin={[pin edge=latex'-, pin-distance=2pt]65:{$c_n\left(1 - \delta^2\right)^n$}}] at (0.7, 0.25) {};
      \node[draw,circle,fill,inner sep=0.5pt,label={270:{$-1$}}] at (-1.2, 0) {};
      \node[draw,circle,fill,inner sep=0.5pt,label={270:{$1$}}] at (1.2, 0) {};
    \end{tikzpicture}
...
\begin{propo}[Moments]\index{Moments}\label{propo:moments}
...
\begin{thm}[Banach-Mazurkiewickz Theorem]\index{Banach-Mazurkiewickz Theorem}\label{thm:banach_mazurkiewickz_theorem}
  Let
  \begin{equation*}
    \ND([0, 1]) = \{ f \in C[0, 1] : f \text{ is nowhere differentiable } \}.
  \end{equation*}
  Then $\ND([0, 1])$ is \hyperref[defn:residual]{residual}\sidenote{For quick reference, a set is residual if its complement is of first category.} in $(C[0, 1], \norm\cdot_\infty)$.
\end{thm}
...
\begin{proof}
  ...
  \end{align*}
  We notice that each of the $\mathcal{F}_n$'s is closed. \hlwarn{incomplete proof, require further work} % TODO complete Banach-Mazurkiewickz
\end{proof}
...
% subsection weierstrass_approximation_theorem_continued (end)

% section the_space_c_x_normcdot_infty_continued (end)

% chapter lecture_32_nov_26th (end)

\chapter{Lecture 33 Nov 28th}%
\label{chp:lecture_33_nov_28th}
% chapter lecture_33_nov_28th

\section{The Space $(C(X), \norm\cdot_\infty)$ (Continued 2)}%
\label{sec:the_space_c_x_normcdot_infty_continued_2}
% section the_space_c_x_normcdot_infty_continued_2

\subsection{Weierstrass Approximation Theorem (Continued 2)}%
\label{sub:weierstrass_approximation_theorem_continued_2}
% subsection weierstrass_approximation_theorem_continued_2

\begin{crly}[Separability of $(C{[a, b]}, {\norm{\cdot}}_\infty)$]\label{crly:separability_of_c_a_b_normcdot_infty}
  $(C[a, b], \norm\cdot_\infty)$ is \hyperref[defn:separable]{separable}.
\end{crly}

\begin{proof}
  Let
  \begin{gather*}
    P_n = \{ a_0 + a_1 x + \hdots + a_n x^n : a_i \in \mathbb{R} \} \\
    Q_n = \{ r_0 + r_1 x + \hdots + r_n x^n : r_i \in \mathbb{Q} \}.
  \end{gather*}
  Then $\bar{Q}_n = P_n$. Then by the \hyperref[thm:weierstrass_approximation_theorem]{Weierstrass Approximation Theorem}, $\bigcup_{n=1}^{\infty} P_n$ is dense, and so is the countable set $\bigcup_{n=1}^{\infty} Q_n$.\qed\
\end{proof}

% subsection weierstrass_approximation_theorem_continued_2 (end)

\subsection{Stone-Weierstrass Theorem}%
\label{sub:stone_weierstrass_theorem}
% subsection stone_weierstrass_theorem

\newthought{Question}: Given a compact metric space $(X, d)$, and a subspace $\Phi \subset C(X)$, how can we tell that $\Phi$ is dense?

From here, we shall always assume that $(X, d)$ is a compact metric space.

\begin{defn}[Point-Separating]\index{Point-Separating}\label{defn:point_separating}
  We say that $\Phi \subset C(X)$ is \hlnoteb{point-separating} if\sidenote{Note that this definition does mean that every $f \in \Phi$ is injective, as the function may depend on either one or both $x$ and $y$. Of course, if every $f \in \Phi$ is injective, then $\Phi$ is, trivially, point-separating.}
  \begin{equation*}
    \forall x, y \in X ( x \neq y \implies \exists f \in \Phi ( f(x) \neq f(y) ) ).
  \end{equation*}
\end{defn}

\begin{propo}[$C(X)$ is Point-Separating]\label{propo:_c_x_is_point_separating}
  $C(X)$ is point-separating.
\end{propo}

\begin{proof}
  Let $a, b \in X$ such that $a \neq b$. Then, define $f_a(x) = d(a, x)$. It is then clear that $f_a \in C(X)$. Since $a \neq b$, we have that $f_a(b) = d(a, b) > 0$.\qed\
\end{proof}

\begin{note}
  Suppose that $\Phi \subset C(X)$, and $x_1, x_2 \in X$ with $x_1 \neq x_2$, such that for any $f \in \Phi$, $f(x_1) = f(x_2)$. Then if $g \in \bar{\Phi}$, we must have $g(x_1) = g(x_2)$\sidenote{For otherwise $g$ would not be continuous}. This shows that if $\Phi$ is dense in $C(X)$, then it must separate points.
\end{note}

\subsubsection{Lattice Version}%
\label{ssub:lattice_version}
% subsubsection lattice_version

\begin{defn}[Lattice]\index{Lattice}\label{defn:lattice}
  A subspace $\Phi \subset C(X)$ is a \hlnoteb{lattice} if $f \lor g, \, f \land g \in \Phi$ for each $f, g \in \Phi$, where\sidenote{In words, a lattice is a set of functions closed under \hlnotea{maxima and minima}.}
  \begin{align*}
...
\begin{note}\label{note:subspace_as_a_lattice}
  \begin{enumerate}
    \item Notice that
      \begin{equation*}
        ( f \lor g )(x) = \frac{(f(x) + g(x)) + \abs{ f(x) - g(x) }}{2} \in C(X)
      \end{equation*}
      for any $f, g \in C(X)$.
    \item For minima, we have
      \begin{equation*}
        ( f \land g )(x) = - ( -f \lor -g ) = \frac{(f(x) + g(x)) - \abs{ f(x) - g(x) }}{2} \in C(X).
      \end{equation*}
  \end{enumerate}
  It follows that since both $f \lor g$ and $f \land g$ are in $C(X)$ that $C(X)$ is a lattice. Moreover, if $\Phi \subset C(X)$ is a linear subspace, then $\Phi$ is a lattice if $f \lor g \in \Phi$ for every $f, g \in \Phi$.
\end{note}

\begin{eg}
  A function $f \in C[a, b]$ is said to be a piecewise linear\index{piecewise linear function} if there exists
  \begin{equation*}
  ...
  \end{equation*}
  The function is \hldefn{piecewise polynomial} if
  \begin{equation*}
...
\begin{thm}[Stone-Weierstrass Theorem --- Lattice Version]\index{Stone-Weierstrass Theorem --- Lattice Version}\label{thm:stone_weierstrass_theorem_lattice_version}
  Let $(X, d)$ be a compact metric space. Let $\Phi$ be a linear subspace of $C(X)$ such that
  \begin{enumerate}
    \item the constant function $1 \in \Phi$ \sidenote{It is okay that we simultaneously have $1 \in \Phi$ and $\Phi$ separating points, for all we need to know that $\Phi$ separate points is that for any $x, y \in X$ with $x \neq y$, \hlimpo{there exists} some $f \in \Phi$ such that $f(x) \neq f(y)$.};
    \item $\Phi$ is point-separating; and
    \item $f \lor g \in \Phi$ for any $f, g \in \Phi$ \sidenote{This implies that $\Phi$ is a lattice by note on page \pageref{note:subspace_as_a_lattice}.}
  \end{enumerate}
  Then $\bar{\Phi}$ is dense in $C(X)$.
\end{thm}

\begin{proof}
  \marginnote{This is likely not a proof that one can come up in one sitting, especially when it is a theory that covers over 2 centuries of mathematical work. \todo{I need to get a better picture of the motivation of the proof.} As it is, it is very difficult to understand how this proof came by, and many of the steps are purely constructive.}Note that given $\alpha, \beta \in \mathbb{R}$ with $a \neq b \in X$, since $\Phi$ is point-separating (2), we can find $\phi \in \Phi$ such that $\phi(a) \neq \phi(b)$. Then, let
  \begin{equation*}
  ...
  Let $f \in C(X)$ and $\epsilon > 0$. Now for any pair $x, y \in X$, we can find $\phi_{x, y} \in \Phi$ such that $\phi_{x, y}(x) = f(x)$ and $\phi_{x, y}(y) = f(y)$\todo{What else can we understand from $\phi_{x, y}$?} \sidenote{I feel somewhat on edge not having the faintest idea how $\phi_{x, y}$ works, except that it separates $x$ and $y$.}. Let $x \in X$. Since $\phi_{x, y}(y) - f(y) = 0$, and both $\phi_{x, y}$ and $f$ are continuous, we can find, for each $y \in X$, a $\delta_y > 0$ such that if $t \in B\left(y, \delta_y\right)$, then
  \begin{equation*}
...
% subsubsection lattice_version (end)

\subsubsection{Subalgebra Version}%
\label{ssub:subalgebra_version}
% subsubsection subalgebra_version

\begin{defn}[Subalgebra]\index{Subalgebra}\label{defn:subalgebra}
...
\begin{eg}
  ...
  \end{equation*}
  is a subalgebra of $C[a, b]$.\sidenote{See a quick work in notes on \href{https://tex.japorized.ink/PMATH347S18/classnotes.pdf\#page160}{PMATH 347}.}
\end{eg}

\begin{lemma}[Closure of a Subalgebra is a Subalgebra]\label{lemma:closure_of_a_subalgebra_is_a_subalgebra}
...
\begin{thmnonum}[Stone-Weierstrass Theorem --- Subalgebra Version]
...
% subsubsection subalgebra_version (end)

% subsection stone_weierstrass_theorem (end)

% section the_space_c_x_normcdot_infty_continued_2 (end)

% chapter lecture_33_nov_28th (end)

\chapter{Lecture 34 Nov 30th}%
\label{chp:lecture_34_nov_30th}
% chapter lecture_34_nov_30th

\section{The Space $(C(X), \norm\cdot_\infty)$ (Continued 3)}%
\label{sec:the_space_c_x_normcdot_infty_continued_3}
% section the_space_c_x_normcdot_infty_continued_3

\subsection{Stone-Weierstrass Theorem (Continued)}%
\label{sub:stone_weierstrass_theorem_continued}
% subsection stone_weierstrass_theorem_continued

\subsubsection{Subalgebra Version (Continued)}%
\label{ssub:subalgebra_version_continued}
% subsubsection subalgebra_version_continued

\begin{thm}[Stone-Weierstrass Theorem --- Subalgebra Version]\index{Stone-Weierstrass Theorem --- Subalgebra Version}\label{thm:stone_weierstrass_theorem_subalgebra_version}
  If $\Phi \subset C(X)$ is a linear subspace such that
  \begin{enumerate}
    \item $1 \in \Phi$;
    \item $\Phi$ is point-separating; and
...
\begin{proof}[\vvimponote]
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      ...
      \draw[->] (0,-0.5) -- (0, 2) node[above] {$y$};
      \draw[-] (-2,1) -- (0, 0) -- (2, 1);
      \draw[dashed] (-2,1.2) -- (0, 0.2) -- (2, 1.2);
      \draw[dashed] (-2,0.8) -- (0, -0.2) -- (2, 0.8);
    \end{tikzpicture}
...
\newthought{Question}: Then what about
\begin{equation*}
  \Phi' = \Span \{ x^2, x^4, \ldots \}?
\end{equation*}
Is $\Phi'$ dense in $C[0, 1]$? No. \sidenote{This was given as a reason but I don't know what exactly does it entail. \todo{I should find out about this.} That said, it is clear that $\Phi'$ separates points, and still a subalgebra, but $1$ can we still create the constant function $1$ in $\Phi$ using only the other generators?}If $f \in \Phi'$, then $f(0) = 0$.
...
\end{equation*}
which is a \hlnotea{closed ideal} in $C[0, 1]$. Then, in particular, we have that for any $g \in C[0, 1]$, we have that $gf, fg \in S$ for any $f \in S$. It can be shown\todo{how?}\sidenote{I should probably work this out on my own.} that
\begin{equation*}
...
\begin{note}
  ...
  \begin{equation*}
    \norm{ f }_\infty = \sup \{ \abs{ f(x) } \mid x \in X \}.
  \end{equation*}
  We say that $\Phi \subset C(X, \mathbb{C})$ is \hldefn{self-adjoint} if
  \begin{equation*}
...
\begin{thm}[Stone-Weierstrass Theorem --- Complex Version]\index{Stone-Weierstrass Theorem --- Complex Version}\label{thm:stone_weierstrass_theorem_complex_version}
...
% subsubsection subalgebra_version_continued (end)

% subsection stone_weierstrass_theorem_continued (end)

% section the_space_c_x_normcdot_infty_continued_3 (end)

% chapter lecture_34_nov_30th (end)

\chapter{Lecture 35 Dec 03rd}%
\label{chp:lecture_35_dec_03rd}
% chapter lecture_35_dec_03rd

\section{The Space $(C(X), \norm\cdot_\infty)$ (Continued 4)}%
\label{sec:the_space_c_x_normcdot_infty_continued_4}
% section the_space_c_x_normcdot_infty_continued_4

\subsection{Compactness in $C(X)$ and the Ascoli-Arzela Theorem}%
\label{sub:compactness_in_c_x_and_the_ascoli_arzela_theorem}
% subsection compactness_in_c_x_and_the_ascoli_arzela_theorem

\newthought{Question}: If $(X, d)$ is compact nad $\mathcal{F} \subset C(X)$, when is $\mathcal{F}$ compact?

We require the following notion:

\begin{defn}[Equicontinuity]\index{Equicontinuity}\label{defn:equicontinuity}
  Let $(X, d)$ be a metric space with $\mathcal{F} \subset C_b(X)$. We say that $\mathcal{F}$ is \hlnoteb{(pointwise) equicontinuous at $x_0 \in X$} if
  \begin{gather*}
    \forall \epsilon > 0 \; \exists \delta_{x_0} > 0 \; \forall f \in \mathcal{F} \; \forall x \in X \\
    d(x, x_0) < \delta_{x_0} \implies \abs{ f(x) - f(x_0) } < \epsilon.
  \end{gather*}
  We say that $\mathcal{F}$ is \hlnoteb{equicontinuous} if it is (pointwise) equicontinous at each $x_0 \in X$.

  We say that $\mathcal{F}$ is \hldefn{uniformly equicontinuous} if
  \begin{gather*}
...
\begin{note}
  Notice that in the definition above, as compared to \hlnoteb{regular continuity} we have
  \begin{enumerate}
    \item for \hlnoteb{continuity}, $\delta$ may depend on $\epsilon$, $f$ and $x_0$;
    \item for \hlnoteb{uniform continuity}, $\delta$ may depend on $\epsilon$ and $f$;
    \item for \hlnoteb{equicontinuity}, $\delta$ may depend on $\epsilon$ and $x_0$; while
    \item for \hlnoteb{uniform equicontinuity}, $\delta$ may solely depend on $\epsilon$.
  \end{enumerate}
  This was outlined on \href{https://en.wikipedia.org/wiki/Equicontinuity}{Wikipedia}\sidenote{So take it with a grain of salt?}.
\end{note}
...
\begin{propo}[Equicontinuity in a Compact Set is Uniform]\label{propo:equicontinuity_in_a_compact_set_is_uniform}
  If $(X, d)$ is compact and if $\mathcal{F} \subset C(X)$ is equicontinuous, then $\mathcal{F}$ is uniformly equicontinuous.
\end{propo}

\begin{proof}
  Let $\epsilon > 0$.
  Since $\mathcal{F}$ is equicontinuous,
  for each $x_0 \in X$, we can find $\delta_{x_0} > 0$ if $x \in B(x_0, \delta_{x_0})$,
  then $\abs{ f(x) - f(x_0) } < \frac{\epsilon}{2}$ for any $f \in \mathcal{F}$.
  Since $(X, d)$ is compact, the cover $\{ B(x_0, \delta_{x_0}) \}_{x_0 \in X}$
  has a \hyperref[thm:lesbesgue]{Lesbesgue Number} $\delta_0 > 0$.
  Then, let $0 < \delta < \delta_0$.
  If for $w , z \in X$ we have $d(w, z) < \delta$,
  then $z \in B(w, \delta) \subset B(x_0', \delta_{x_0'})$ for some $x_0' \in X$.
  Then
  \begin{equation*}
    \abs{ f(z) - f(w) } \leq \abs{ f(z) - f(x_0) } + \abs{ f(x_0) - f(w) } < \epsilon.
  \end{equation*}\qed\
\end{proof}

\begin{defn}[Pointwise Bounded Functions]\index{Pointwise Bounded Functions}\label{defn:pointwise_bounded_functions}
  A family of functions $\mathcal{F} \subset C_b(X)$ is \hlnoteb{pointwise bounded} if for each $x_0 \in X$, $\exists M_{x_0} > 0$ such that $\abs {f(x_0)} < M_{x_0}$ for every $f \in \mathcal{F}$. We say that $\mathcal{F}$ is \hlnoteb{uniformly bounded}\index{Uniformly Bounded Functions} if $\exists M > 0$ such that $\norm{f}_\infty \leq M$ for every $f \in \mathcal{F}$.
\end{defn}

\begin{propo}[Pointwise Bounded Equicontinuous Functions in a Compact Set are Uniformly Bounded]\label{propo:pointwise_bounded_equicontinuous_functions_in_a_compact_set_are_uniformly_bounded}
  Assume that $(X, d)$ is compact and that $\mathcal{F} \subseteq C(X)$ is equicontinuous and pointwise bounded. Then $\mathcal{F}$ is uniformly bounded.
\end{propo}

\begin{proof}
  By \cref{propo:equicontinuity_in_a_compact_set_is_uniform}, $\mathcal{F}$ is uniformly equicontinuous. So let $\epsilon = 1$. Then $\exists \delta > 0$ such that for any $x, y \in X$, if $y \in B(x, \delta)$, then $\abs{ f(x) - f(y) } < 1$ for any $f \in \mathcal{F}$. By compactness of $(X, d)$ there exists a finite subset $\{ x_1, \ldots, x_n \} \subset X$ such that
  \begin{equation*}
    X = \bigcup_{i=1}^{n} B(x_i, \delta).
  \end{equation*}
  By assumption, we also know that for each of these $x_i$'s, there exists $M_1, \ldots, M_n > 0$ such that for any $f \in \mathcal{F}$, $\abs{ f(x_i) } \leq M_i$. Then let
  \begin{equation*}
    M_0 = \max \{ M_1, \ldots, M_n \}.
  \end{equation*}
  Then for any $z \in X$, we have that $z \in B(x_{i_0}, \delta)$ for some $i_0$. Therefore, we have that
  \begin{equation*}
    \abs{f(z)} \leq \abs{ f(z) - f(x_{i_0}) } + \abs{ f(x_{i_0}) } < 1 + M_0.
  \end{equation*}\qed\
\end{proof}

\begin{defn}[Relatively Compact Sets]\index{Relatively Compact Sets}\label{defn:relatively_compact_sets} Let $A \subset (X, d)$. We say that $A$ is \hlnoteb{relatively compact} if $\bar{A}$ is compact.
\end{defn}

\begin{note}
  If $(X, d)$ is complete, then we have that $A$ is relatively compact iff $A$ is totally bounded.
\end{note}

\begin{thm}[Arzel\`{a}-Ascoli]\index{Arzel\`{a}-Ascoli}\label{thm:arzela_ascoli}
  Let $(X, d)$ be a compact metric space, and $\mathcal{F} \subset C(X)$. TFAE:
  \begin{enumerate}
    \item $\mathcal{F}$ is relatively compact.
    \item $\mathcal{F}$ is equicontinous and pointwise-bounded.
  \end{enumerate}
\end{thm}

\begin{proof}
  \hlbnoted{$(1) \implies (2)$} Since $(X, d)$ is compact, it is complete, and so $\mathcal{F}$ being relatively compact implies that $\mathcal{F}$ is totally bounded. Thus $\mathcal{F}$ has a finite $\frac{\epsilon}{3}$-net $\{ f_1, f_2, \ldots, f_n \} \subset \mathcal{F}$. By an earlier example, we have that $\{ f_1, f_2, \ldots, f_n \}$ is equicontinuous, and hence uniformly equicontinuous by \cref{propo:equicontinuity_in_a_compact_set_is_uniform}. By that, we can find a $\delta > 0$ such that $\forall x, y \in X$, if $d(x, y) < \delta$, we have
  \begin{equation*}
  ...
  \begin{marginfigure}
    ...
    \begin{tikzpicture}
      \draw[-] (-1, -2) -- (-1, 2);
      \draw[-] (1, -2) -- (1, 2);
      \foreach \x in {1.6,1.3,...,-1.6}
      { \draw[-] (-1.1,\x) -- (-0.9,\x);
        \draw[-] (0.9,\x) -- (1.1,\x); }
      \node[label={0:{$M$}}] at (1.1,1.6) {};
      \node[label={0:{$-M$}}] at (1.1,-1.4) {};
      \node[label={0:{$y_j$}}] at (1.1,0.7) {};
      \node[label={0:{$y_{j - 1}$}}] at (1.1,0.4) {};
      \draw[<->] (1.9,0.7) -- (1.9,0.4) node[midway,right] {$\frac{\epsilon}{3}$};

      \node[label={180:{$x_1$}}] at (-1.1,1.6) {};
      \node[label={180:{$x_2$}}] at (-1.1,1.3) {};
      \node[label={180:{$x_n$}}] at (-1.1,-1.4) {};
      \draw[<->] (-1.7,1.6) -- (-1.7,1.3) node[midway,left] {$\delta$};
    \end{tikzpicture}
...
% subsection compactness_in_c_x_and_the_ascoli_arzela_theorem (end)

% section the_space_c_x_normcdot_infty_continued_4 (end)

% chapter lecture_35_dec_03rd (end)
...
% chapter assignment_2 (end)

\chapter{Assignment 3}%
\label{chp:assignment_3}
% chapter assignment_3
