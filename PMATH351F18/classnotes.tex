...
Note that if we allow for $0 < i < 1$, then we would have a graph that looks like the following:
\begin{center}
\begin{tikzpicture}
  \draw[-latex'] (-2,0) -- (2, 0);
  \draw[-latex'] (0,-2) -- (0, 2);
...
\end{tikzpicture}
\end{center}
which is a convex graph, i.e. we cannot create well-defined norms.
}
