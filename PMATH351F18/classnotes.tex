...
\begin{propo}[Properties of Open Sets]
...
\begin{crly}[Properties of Closed Sets]
...
\begin{note}
  On $\mathbb{R}$, only $\emptyset$ and $\mathbb{R}$ itself are both open and closed. This can be proven using the \hldefna{Intermediate Value Theorem}.
\end{note}

\begin{defn}[Topology]\index{Topology}
\label{defn:topology}
...
\begin{thm}[Open Balls are Open]
\label{thm:open_balls_are_open}
  \begin{enumerate}
...
\begin{proof}
  \begin{enumerate}
    ...
    \item Let $U \subset X$ be open. $\forall x \in U$, let $\epsilon_x > 0$ be such that $B(x, e\spilon_x) \subset U$. Then
      \begin{equation*}
