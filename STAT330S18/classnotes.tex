...
\begin{defn}[Independence]\index{Independence}
...
\label{thm:independence_by_pf}
  ...
  Suppose $X$ and $Y$ are rvs with joint cdf $F$, joint pf $f$, marginal cdf $F_X$ and $F_Y$ respectively, and marginal pf $f_X$ and $f_Y$ respectively. Also, suppoes that $A_X = \{x : f_X (x) > 0\}$ is the support set of $X$ and $A_Y = \{y : f_Y(y) > 0\}$ is the support set of $Y$. Then $X$ and $Y$ are independent rvs if and only if either 
  \begin{equation*}
...
\begin{proof}
  The $(\implies)$ direction is simply a result of \hlnotea{Clairaut's Theorem}. While the $(\impliedby)$ direction is a direct result of applying double integrals. \qed
\end{proof}
...
\begin{eg}[Example 3.6]
  ...
  \begin{solution} 
    ...
    \end{equation*}
    and so this suggests that we can indeed break down $f$ into two parts, each only affected by $x$ and $y$ respectively, ``indepdent'' of each other. Indeed, since
    \begin{align*}
